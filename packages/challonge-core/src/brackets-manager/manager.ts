import { type CrudInterface, type Database, type Storage } from "./types";
import { type InputStage, type Stage } from "../brackets-model/index";
	...
	constructor(storageInterface: CrudInterface, verbose?: boolean) {
		...
		this.storage.selectFirst = async (table, filter, assertUnique = true) => {
			...
			if (assertUnique && results.length > 1)
				throw Error(
					`Selecting ${JSON.stringify(filter)} on table "${table}" must return a unique value.`,
				);
		...
		for (const method of Object.getOwnPropertyNames(
			...
			storage[method] = async (
				...
				if (verbose) {
					...
					start = Date.now();
					console.log(
						`${id} ${method.toUpperCase()} "${table}" args: ${JSON.stringify(args)}`,
					);
				}
				...
				if (verbose) {
					...
					// eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
					console.log(
						`${id!} ${duration}ms - Returned ${JSON.stringify(result)}`,
					);
				}
