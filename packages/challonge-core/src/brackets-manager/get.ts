...
export class Get extends BaseGetter {
  ...
  public async matchGames(matches: Match[]): Promise<MatchGame[]> {
    ...
    const matchGamesQueries = await Promise.all(
      parentMatches.map((match) => this.storage.select("match_game", { parent_id: match.id })),
    );
  ...
  public async currentRound(stageId: Id): Promise<Round | null> {
    ...
    for (const roundMatches of matchesByRound) {
      if (roundMatches.every((match) => match.status >= Status.Completed)) continue;
  ...
  public async currentMatches(stageId: Id): Promise<Match[]> {
    ...
    for (const roundMatches of matchesByRound) {
      ...
      if (stage.settings.consolationFinal && currentRoundIndex === roundCount - 1) {
        const [final] = roundMatches as [Match, ...Match[]];
        const [consolationFinal] = matchesByRound[currentRoundIndex + 1]! as [Match, ...Match[]];

        const finals = [final, consolationFinal];
        if (finals.every((match) => !helpers.isMatchOngoing(match))) return currentMatches;
  ...
  public async finalStandings(
    stageId: Id,
    roundRobinOptions?: RoundRobinFinalStandingsOptions,
  ): Promise<FinalStandingsItem[] | RoundRobinFinalStandingsItem[]> {
    const stage = await this.storage.select("stage", stageId);
    if (!stage) throw Error("Stage not found.");
  ...
  private async singleEliminationStandings(stage: Stage): Promise<FinalStandingsItem[]> {
    ...
    const [singleBracket, finalGroup] = groups as [Group, Group | undefined];
  ...
  private async doubleEliminationStandings(stage: Stage): Promise<FinalStandingsItem[]> {
    ...
    const [winnerBracket, loserBracket, finalGroup] = groups as [Group, Group, Group | undefined];
