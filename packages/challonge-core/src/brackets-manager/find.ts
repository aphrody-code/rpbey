import { type Group, type Id, type Match, type MatchGame } from "../brackets-model/index";
...
export class Find extends BaseGetter {
  ...
  public async nextMatches(matchId: Id, participantId?: number): Promise<Match[]> {
    ...
    if (participantId !== undefined) {
      ...
      if (stage.type === "single_elimination" && loser?.id === participantId) return []; // Eliminated.
      ...
      if (stage.type === "double_elimination") {
        ...
        const { winnerBracketMatch, loserBracketMatch, finalGroupMatch } =
        ...
        if (matchLocation === "loser_bracket") {
          ...
          if (participantId === winner?.id) return loserBracketMatch ? [loserBracketMatch] : [];
          ...
          if (participantId === winner?.id) return winnerBracketMatch ? [winnerBracketMatch] : [];
    ...
    for (const match of matches) {
      ...
      matchByGroupType = {
        winnerBracketMatch:
          ...
          (helpers.isWinnerBracket("double_elimination", group.number) ? match : undefined),
        loserBracketMatch:
          matchByGroupType["loserBracketMatch"] ??
          (helpers.isLoserBracket("double_elimination", group.number) ? match : undefined),
        finalGroupMatch:
          matchByGroupType["finalGroupMatch"] ??
          (helpers.isFinalGroup("double_elimination", group.number) ? match : undefined),
      };
    }
