...
import { type DeepPartial, type ParticipantSlot, type Side } from "../types";
...
export class BaseUpdater extends BaseGetter {
  ...
  protected async updateSeeding(
    stageId: Id,
    { seeding, seedingIds }: { seeding?: Seeding | null; seedingIds?: IdSeeding | null },
  ...
  protected static assertCanUpdateSeeding(matches: Match[], slots: ParticipantSlot[]): void {
    ...
    for (const match of matches) {
      ...
      if (
        isParticipantLocked &&
        (match.opponent1?.id !== opponent1?.id || match.opponent2?.id !== opponent2?.id)
      )
  ...
  protected async updatePrevious(
    ...
    if (match.status >= Status.Running) await this.archiveMatches(previousMatches);
  ...
  protected async updateNext(
    ...
    if (nextMatches.length === 0) {
      ...
      if (match.status === Status.Completed) await this.archiveMatches([match]);
