...
export class StageCreator {
  ...
  private async singleElimination(): Promise<Stage> {
    if (
      !this.stage.settings?.manualOrdering &&
  ...
  private async createMatch(
    ...
    if (this.updateMode) {
      ...
      if (existing) {
        ...
        if (existingStatus > status && existingStatus >= Status.Running) status = existingStatus;
      }
    }
  ...
  public async getSlots(positions?: Seed[]): Promise<ParticipantSlot[]> {
    ...
    const slots =
      this.stage.seedingIds !== undefined || helpers.isSeedingWithIds(seeding)
