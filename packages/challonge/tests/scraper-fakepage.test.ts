...
describe("ChallongeScraper — constructor + lifecycle", () => {
	test("does not spawn Chromium on construction", () => {
		...
		expect(
			logs.filter((l) => l.toLowerCase().includes("browser")),
		).toHaveLength(0);
		// Transport should be null until first use
		expect(
			(scraper as unknown as Record<string, unknown>)["transport"],
		).toBeNull();
	});
...
describe("FakePage — offline mock", () => {
	...
	beforeEach(async () => {
		...
		const t = (
			scraper as unknown as { getTransport: () => unknown }
		).getTransport();
		// Mock fetch
	...
	test("evaluate(() => document.title) throws with clear message", async () => {
		await expect(
			page.evaluate(
				() => (document as unknown as Record<string, unknown>)["title"],
			),
		).rejects.toThrow(
			"page.evaluate(<arbitrary JS>) is not supported in bxc mode",
		);
	});

	test("evaluate(arbitrary expression string) throws CurlImpersonateError", async () => {
		await expect(
			page.evaluate("document.querySelectorAll('tr')"),
		).rejects.toBeInstanceOf(CurlImpersonateError);
	});
...
describe("parseStoreState — offline", () => {
	test("extracts all store keys from FAKE_HTML via openPage.evaluate", async () => {
		...
		const store = (await p.evaluate("window._initialStoreState")) as Record<
			string,
			unknown
		>;
...
describe("ChallongeScraper — network (skipped when FFI unavailable)", () => {
	skipIfNoLib("openPage(url).content() returns non-empty HTML", async () => {
		const scraper = new ChallongeScraper({ log: () => {} });
		try {
			const page = await scraper.openPage(
				"https://challonge.com/fr/B_TS5/module",
			);
			const html = await page.content();
			expect(typeof html).toBe("string");
...
describe("dumpChallongeRaw — network (skipped when FFI unavailable)", () => {
	skipIfNoLib(
		"returns html, store, and parsed snapshot for /module",
		async () => {
			...
			expect(result.parsed === null || typeof result.parsed === "object").toBe(
				true,
			);
		},
	);
