...
const COOKIE_PATH =
	join(os.homedir(), "bxc/cookies/private/challonge.json");
...
describe("upgradeToHttps", () => {
	test("upgrades http:// to https://", () => {
		expect(upgradeToHttps("http://challonge.com/")).toBe(
			"https://challonge.com/",
		);
	});

	test("leaves https:// unchanged", () => {
		expect(upgradeToHttps("https://challonge.com/")).toBe(
			"https://challonge.com/",
		);
	});
});
...
describe("isPermittedRedirect", () => {
	test("same host is permitted", () => {
		expect(
			isPermittedRedirect("https://challonge.com/a", "https://challonge.com/b"),
		).toBe(true);
	});
	...
	test("www. toggle is permitted", () => {
		expect(
			isPermittedRedirect(
				"https://challonge.com/a",
				"https://www.challonge.com/b",
			),
		).toBe(true);
	});
	...
	test("cross-host is not permitted", () => {
		expect(
			isPermittedRedirect("https://challonge.com/a", "https://evil.com/b"),
		).toBe(false);
	});

	test("protocol change is not permitted", () => {
		expect(
			isPermittedRedirect("https://challonge.com/a", "http://challonge.com/b"),
		).toBe(false);
	});
});
...
describe("BxcTransport — live fetch (challonge.com)", () => {
	...
	test("fetches /log page, status 200, body contains _initialStoreState, time < 5s", async () => {
		...
		try {
			const res = await transport.fetch(
				"https://challonge.com/fr/B_TS5/log?page=1",
			);
	...
	test("curlImpersonateGet facade delegates to BxcTransport", async () => {
		...
		const res = await curlImpersonateGet(
			"https://challonge.com/fr/B_TS5/log?page=1",
			{
				cookiePath: COOKIE_PATH,
				cache: false,
