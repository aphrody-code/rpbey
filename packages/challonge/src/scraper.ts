...
function storeToLogEntries(store: Record<string, unknown>): ScrapedLogEntry[] {
	...
	const rawEntries =
		...
		((store["TournamentStore"] as Record<string, unknown> | null)?.["log"] as
			| LogEntryRaw[]
			| null) ??
		((store["TournamentStore"] as Record<string, unknown> | null)?.[
			"activity_log"
		] as LogEntryRaw[] | null);
...
): ActivityFeedSettings | null {
	const s = store["ActivityFeedSettingsStore"] as Record<
		string,
		unknown
	> | null;
	if (!s) return null;
...
function mapSnapshotToScrapedTournament(
	...
	const participants: ScrapedParticipant[] = Array.from(
	...
	const ranksSet = new Set(
		participants.map((p) => p.finalRank).filter((r) => r != null),
	);
	if (
		ranksSet.size <= 2 &&
	...
	return {
		metadata: {
			...
			startedAt: toIso(
				(snap.tournament as unknown as Record<string, unknown>)["started_at"],
			),
			completedAt: toIso(
				(snap.tournament as unknown as Record<string, unknown>)["completed_at"],
			),
		},
...
function storeToStandings(store: Record<string, unknown>): ScrapedStanding[] {
	...
	const raw: unknown[] =
		(ss?.["standings"] as unknown[] | null) ??
		(ts?.["standings"] as unknown[] | null) ??
		[];

	return (raw as Record<string, unknown>[]).map((s, i) => ({
		rank: (s["rank"] as number) ?? (s["final_rank"] as number) ?? i + 1,
		name: ((s["display_name"] as string) ?? (s["name"] as string) ?? "")
			.trim()
			.replace("✅", ""),
		challongeUsername:
			(s["username"] as string | null) ??
			(s["challonge_username"] as string | null) ??
			null,
...
export class ChallongeScraper {
	...
	async openPage(url: string, _signal?: AbortSignal): Promise<FakePage> {
		...
		const page: FakePage = {
			...
			async evaluate(fn: unknown): Promise<unknown> {
				...
				const isStoreAccess =
					fnStr.includes("window._initialStoreState") ||
					fnStr.includes("_initialStoreState");
	...
	async scrape(
		...
		if (withParticipants) {
			tasks.push(
				this.fetchParticipants(slug, signal)
					...
					.catch((err) =>
						this.opts.log(
							`[ChallongeScraper] /participants error: ${(err as Error).message}`,
						),
					),
			);
		}
		...
		if (withStandings) {
			tasks.push(
				this.fetchStandings(slug, signal)
					...
					.catch((err) =>
						this.opts.log(
							`[ChallongeScraper] /standings error: ${(err as Error).message}`,
						),
					),
			);
		}
		...
		if (withStations) {
			...
			this.opts.log(
				`[ChallongeScraper] /stations not yet supported in bxc mode, returning []`,
			);
		}
		...
		if (withLog) {
			tasks.push(
				this.fetchLog(slug, signal)
					...
					.catch((err) =>
						this.opts.log(
							`[ChallongeScraper] /log error: ${(err as Error).message}`,
						),
					),
			);
		}
