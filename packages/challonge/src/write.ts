/**
 * Challonge API v2.1 client — WRITE surface (OAuth 2.0 client_credentials).
 *
 * Complementary to `api.ts` (v1 READ, HTTP Basic). Where `api.ts` reads the
 * legacy v1 JSON shapes without bot challenges, this client speaks the modern
 * JSON:API v2.1 surface (`https://api.challonge.com/v2.1`) and performs the
 * mutating operations the tournament lifecycle needs:
 *  - tournaments: list / get / create / change_state / delete
 *  - participants: list / create / bulk_add / delete / randomize / check-in
 *  - matches: list / get / update (report scores) / mark underway
 *
 * Auth (two modes, selected by `authType`):
 *  - "v2" → OAuth 2.0 Client Credentials. POST `/oauth/token` with
 *    `grant_type=client_credentials`; the bearer token is cached until it
 *    nears expiry (5 min margin) so subsequent requests reuse it.
 *  - "v1" → legacy API key sent verbatim in `Authorization`.
 * Every request carries the `Authorization-Type` header (v1|v2) per the v2.1
 * contract, `Content-Type: application/vnd.api+json`, and `Accept: application/json`.
 *
 * Credentials are read from the environment by the `getChallongeClient()`
 * factory: `CHALLONGE_CLIENT_ID` + `CHALLONGE_CLIENT_SECRET` (preferred, OAuth)
 * else `CHALLONGE_API_KEY` (v1 fallback).
 *
 * Network: `globalThis.fetch` only — no bxc — so this module stays bundlable.
 * All timestamps surfaced through these types are ISO strings (challonge side).
 */
...
export class ChallongeClient {
  ...
  private async getOAuthToken(): Promise<string> {
    ...
    if (!this.clientId || !this.clientSecret) {
      throw new Error("CHALLONGE_CLIENT_ID et CHALLONGE_CLIENT_SECRET requis pour OAuth 2.0");
    }
  ...
  async deleteParticipant(tournamentId: string, participantId: string): Promise<void> {
    await this.request("DELETE", `/tournaments/${tournamentId}/participants/${participantId}`);
  }
  ...
  async randomizeParticipants(tournamentId: string): Promise<void> {
    await this.request("POST", `/tournaments/${tournamentId}/participants/randomize`);
  }
