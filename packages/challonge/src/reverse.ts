...
import {
	BxcTransport,
	type BxcTransportOptions,
	type BxcFetchOptions,
} from "./transports/bxc";
import {
	isRedirectInfo,
...
export class ChallongeReverse {
	...
	constructor(options: ChallongeReverseOptions = {}) {
		...
		const transportOpts: BxcTransportOptions = {
			...
			timeoutMs:
				options.timeoutSec != null ? options.timeoutSec * 1000 : undefined,
	...
	async getLogPage(slug: string, page = 1): Promise<LogPageResult> {
		...
		const entriesRoot = getReactRoot<LogEntriesProps>(
			pageData.body,
	...
	async getParticipants(slug: string): Promise<{
		...
		return {
			...
			rankings:
				parseJsonAttr<Array<Record<string, unknown>>>(raw.rankings) ?? [],
			raw,
		};
	}
 ...
 */
function extractInitialStoreState(
	html: string,
): Record<string, unknown> | null {
	// Pattern: window._initialStoreState = { ... }; (object literal — old layout)
	const literalMatch =
		/window\._initialStoreState\s*=\s*(\{[\s\S]*?\});\s*<\/script>/.exec(html);
	if (literalMatch) {
		try {
	...
	const parseMatch =
		/window\._initialStoreState\s*=\s*JSON\.parse\s*\(\s*(['"`])([\s\S]*?)\1\s*\)/.exec(
			html,
		);
	if (parseMatch) {
		try {
