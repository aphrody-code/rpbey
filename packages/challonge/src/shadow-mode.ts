...
export function deepDiff(
	...
	if (Array.isArray(primary) && Array.isArray(shadow)) {
		...
		for (let i = 0; i < len; i++) {
			diffs.push(
				...deepDiff(primary[i], shadow[i], `${path}[${i}]`, depth + 1),
			);
		}
		return diffs;
	}
	...
	if (isPlainObject(primary) && isPlainObject(shadow)) {
		const diffs: DiffEntry[] = [];
		const keys = new Set([
			...Object.keys(primary as object),
			...Object.keys(shadow as object),
		]);
	...
	return [
		{ path: path || "(root)", primaryValue: primary, shadowValue: shadow },
	];
}
