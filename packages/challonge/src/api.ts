...
export class ChallongeApi {
	...
	constructor(options: ChallongeApiOptions = {}) {
		...
		if (!apiKey) {
			throw new Error(
				"ChallongeApi: missing API key (set options.apiKey or CHALLONGE_API_KEY).",
			);
		}
		this.apiKey = apiKey;
	...
	async list(
		...
		const arr = await this.request<
			Array<{ tournament: ChallongeApiTournament }>
	...
	toCanonical(
		...
		const participants: ScrapedParticipant[] = partsRaw
			.map((p) => ({
				...
				challongeProfileUrl: p.username
				...
				clinched: !!p.clinch,
				metadata: (p.custom_field_response ?? p.metadata ?? null) as Record<
					string,
					unknown
				> | null,
			}))
		...
		const matches: ScrapedMatch[] = matchesRaw.map((m) => {
			...
			return {
				...
				round,
				bracketSide: bracketSideFromRound(
					round,
					t.tournament_type,
					round === maxPositiveRound,
				),
		...
		const log = opts.synthesizeLog
			? synthesizeLogFromMatches(matches, byId)
			: [];
