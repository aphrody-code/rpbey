...
const PKG_DIR = resolve(import.meta.dir, "../../../node_modules/@rpbey/brackets-viewer/dist");
const DEST_DIR = resolve(import.meta.dir, "../public/vendor/brackets");

const ASSETS = ["brackets-viewer.min.js", "brackets-viewer.min.css", "stage-form-creator.min.js"];

async function main(): Promise<void> {
  const pkgFile = Bun.file(`${PKG_DIR}/brackets-viewer.min.js`);
  if (!(await pkgFile.exists())) {
    console.error(`[copy-brackets-assets] missing source ${PKG_DIR}. Run \`bun install\`.`);
    process.exit(1);
  }
