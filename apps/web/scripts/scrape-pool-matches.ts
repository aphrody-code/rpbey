...
async function main() {
	...
	try {
		while (pageNumber <= maxPages) {
			...
			try {
				await page
					.waitForSelector(".activity-feed-item", { timeout: 10_000 })
					.catch(() => {});
	...
	console.log(
		`   bracket déjà présent: ${allMatches.length - poolMatches.length}`,
	);
	...
	for (let i = 0; i < poolMatches.length; i++) {
		...
		if (!partIdByName.has(wKey) && !partIdByName.has(lKey)) {
			console.log(
				`   ⚠️  skip (no participant matched) ${m.winner} vs ${m.loser}`,
			);
			mSkipped++;
			continue;
		}
