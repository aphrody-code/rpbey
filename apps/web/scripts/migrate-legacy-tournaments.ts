...
async function main() {
	...
	for (const { dir, source } of SOURCES) {
		...
		const valid = files.filter(
			(f) =>
				f.endsWith(".json") && !f.includes(".bak-") && !f.endsWith(".raw.json"),
		);

		console.log(
			`[migrate-legacy-tournaments] ${source}: ${valid.length} files`,
		);
	...
	console.log(
		`[migrate-legacy-tournaments] OK upserted=${total} totalDB=${dbCount}`,
	);
}
