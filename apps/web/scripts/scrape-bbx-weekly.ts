...
async function scrapePeriod(
	...
	const raw = await page.evaluate(() => {
		...
		const out: { category: string; comps: { rank: number; name: string; score: number; dir: number; isNew: boolean }[] }[] = [];
		let current: (typeof out)[number] | null = null;

		// Parcours en ordre document : titres de catégorie + lignes de rang.
		const nodes = document.querySelectorAll(
			'[data-slot="card-title"], span.min-w-8',
		);
...
for (const c of fourWeeks.categories)
	console.log(`  ${c.category}: ${c.components.slice(0, 3).map((x) => `${x.name}(${x.score})`).join(", ")}…`);
