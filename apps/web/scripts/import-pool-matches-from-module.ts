...
async function main() {
	...
	if (!slug || !tournamentId) {
		console.error(
			`Usage: bun scripts/import-pool-matches-from-module.ts <slug>`,
		);
		process.exit(1);
	}
	...
	const files = (await readdir(dumpDir))
		.filter(
			(f) => f.startsWith(`${slug}_module_`) && f.endsWith(".groups.json"),
		)
		.sort()
		.reverse();
	if (files.length === 0) {
		console.error(
			`❌ Aucun ${slug}_module_*.groups.json — run parse-module-html.ts first.`,
		);
		process.exit(1);
	}
