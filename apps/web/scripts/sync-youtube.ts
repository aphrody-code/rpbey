...
async function fetchChannel(channelId: string): Promise<RssVideo[]> {
  ...
  return entries
    .map((e: any): RssVideo | null => {
      ...
      const stats = grp?.["media:community"]?.[0]?.["media:statistics"]?.[0]?.$;
      return {
        id,
        title: e.title?.[0] ?? grp?.["media:title"]?.[0] ?? "(sans titre)",
        url: e.link?.[0]?.$?.href ?? `https://www.youtube.com/watch?v=${id}`,
        thumbnail:
          grp?.["media:thumbnail"]?.[0]?.$?.url ?? `https://i.ytimg.com/vi/${id}/hqdefault.jpg`,
...
async function main() {
  ...
  for (const ch of CHANNELS) {
    ...
    const channelName = videos[0]?.channelName || nameByChannel.get(ch.id) || ch.fallbackName;
