...
async function main() {
	...
	if (scraped) {
		...
		const completeBracket = bracketMatches.filter(
			(m) => m.state === "complete",
		);

		console.log(`\n📊 Breakdown matches:`);
		console.log(
			`   pools  : ${poolMatches.length} (complete: ${completePool.length})`,
		);
		console.log(
			`   bracket: ${bracketMatches.length} (complete: ${completeBracket.length})`,
		);
		console.log(
		...
		await writeFile(dumpPath, JSON.stringify(scraped, null, 2), "utf-8");
		console.log(`\n💾 Dump: ${dumpPath}`);
	}
	...
	if (scraped) {
		...
		const matchedUserCount = [...challongeIdToUser.values()].filter(
			Boolean,
		).length;
		console.log(
			`👤 Users matched: ${matchedUserCount}/${scraped.participants.length}`,
		);
