...
async function importBts(n: number): Promise<{
  ...
  const niceName =
    meta.name && meta.name !== "Tournoi Importé" ? meta.name : `BEY-TAMASHII SÉRIES #${n}`;
  ...
  for (const m of matches) {
    ...
    if (m.winnerId != null) winsBy.set(m.winnerId, (winsBy.get(m.winnerId) ?? 0) + 1);
    if (m.loserId != null) lossesBy.set(m.loserId, (lossesBy.get(m.loserId) ?? 0) + 1);
  }
  ...
  await prisma.$transaction([
    ...
    prisma.tournamentMatch.createMany({
      data: matches.map((m) => ({
        ...
        round: m.round,
        player1Name: m.player1Id != null ? (idToName.get(m.player1Id) ?? null) : null,
        player2Name: m.player2Id != null ? (idToName.get(m.player2Id) ?? null) : null,
        winnerName: m.winnerId != null ? (idToName.get(m.winnerId) ?? null) : null,
        score: m.scores ?? null,
        state: m.state,
      })),
      skipDuplicates: true,
...
async function main() {
  ...
  for (const n of [1, 2, 3, 4]) {
    try {
      ...
      console.error(`✗ BTS${n} :`, err instanceof Error ? err.message : String(err));
    }
  }
