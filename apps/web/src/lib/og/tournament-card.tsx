...
import type { Id, Match, Participant, Stage, StageType, ViewerData } from "@/lib/brackets/types";
...
function isStageLive(matches: Match[]): boolean {
  return matches.some((m) => m.status === Status.Running || m.status === Status.Ready);
}
...
function buildEliminationPodium(
  ...
  if (completed.length === 0) return { champion: null, runnerUp: null, semiFinalists: [] };
...
function MedalCard({
  ...
  const medalColor = rank === 1 ? palette.gold : rank === 2 ? palette.silver : palette.bronze;
  return (
    <div
      style={{
        display: "flex",
...
function EmptyState({ message, palette }: { message: string; palette: OgPalette }) {
  return (
    <div
      style={{
        display: "flex",
...
function buildCardJsx(opts: RenderOptions): React.ReactElement {
  ...
  if (stageType === "round_robin") {
    const standings = buildPoolStandings(opts.data.matches, opts.data.participants);
    const groupIds = [...standings.keys()].sort((a, b) => Number(a) - Number(b));
    ...
    if (!podium.champion && !podium.runnerUp && podium.semiFinalists.length === 0) {
      body = <EmptyState message="Tournoi pas encore lance" palette={palette} />;
    } else {
      ...
      body = (
        ...
        >
          ...
          {lower.length > 0 ? (
            ...
            >
              {lower.map((c, i) => (
                <MedalCard key={i} rank={c.rank} name={c.name} label={c.label} palette={palette} />
              ))}
            </div>
          ) : null}
        </div>
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ display: "flex" }}>challonge.com/{opts.source.idOrSlug}</span>
...
export async function renderTournamentError(opts: RenderErrorOptions): Promise<Buffer> {
  ...
  const sub = opts.idOrSlug ? `Slug "${opts.idOrSlug}" — ${opts.message}` : opts.message;
 ...
 */
export async function renderTournamentCard(opts: RenderOptions): Promise<Buffer> {
