...
import type { ScrapedLogEntry, ScrapedStation, ScrapedTournament } from "./types";
  ...
  constructor(options: ChallongeClientOptions = {}) {
    ...
    this.apiClient =
      options.api === false
        ? null
        : safeNew(
            () => new ChallongeApi((options.api as ChallongeApiOptions | undefined) ?? {}),
            this.log,
            "API disabled",
          );
    this.scraperOpts =
  ...
  async fetch(idOrSlug: string | number, options: FetchOptions = {}): Promise<ScrapedTournament> {
    ...
    if (this.apiClient) {
      try {
        ...
        if (err instanceof ChallongeApiError && (err.status === 401 || err.status === 404)) {
          this.log(`⚠️  API ${err.status} for "${idOrSlug}", falling back to scraper.`);
        } else {
          this.log(`⚠️  API failure (${(err as Error).message}), falling back to scraper.`);
        }
      }
    }
    ...
    if (needScrape && this.scraperOpts !== null) {
      try {
        const scraper = this.getScraper();
        const slug =
          canonical?.metadata.url.replace(/^https?:\/\/challonge\.com\//, "") ?? String(idOrSlug);
        const scrapeResult = await scraper.scrape(slug, {
          ...options,
          // If we already have API data, only fetch what's missing
          withParticipants: canonical ? false : (options.withParticipants ?? true),
        });
...
function safeNew<T>(factory: () => T, log: (msg: string) => void, hint: string): T | null {
  try {
    return factory();
  } catch (err) {
...
function mergeScrapeIntoCanonical(
  ...
  return {
    ...canonical,
    standings: options.withStandings === false ? canonical.standings : scrape.standings,
    stations: options.withStations === false ? canonical.stations : scrape.stations,
