...
function decodeEntities(s: string): string {
  return s.replace(/&(?:quot|apos|amp|lt|gt|#39|#x27|#x2F);/g, (m) => ENTITY_DECODE[m] ?? m);
}
...
export function getReactRoot<T = unknown>(html: string, className: string): ReactRoot<T> | null {
  return extractReactRoots<T>(html).find((r) => r.className === className) ?? null;
}
 ...
 */
export function readDataAttrs(html: string, selector = "body"): Record<string, string> {
