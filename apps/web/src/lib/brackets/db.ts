...
export function bracketDbToViewerData(
	...
	const wbRounds = [
		...new Set(bracketMatches.filter((m) => m.round > 0).map((m) => m.round)),
	].sort((a, b) => a - b);
	const lbRounds = [
		...new Set(bracketMatches.filter((m) => m.round < 0).map((m) => m.round)),
	].sort((a, b) => b - a); // -1, -2, -3 ... → ordre logique
	...
	for (const m of bracketMatches) {
		...
		const p1Id = m.player1Name
			? participantIdByName.get(m.player1Name.toLowerCase())
			: null;
		const p2Id = m.player2Name
			? participantIdByName.get(m.player2Name.toLowerCase())
			: null;
		const winnerId = m.winnerName
			? participantIdByName.get(m.winnerName.toLowerCase())
			: null;
		...
		const opp1Result =
			winnerId && p1Id ? (winnerId === p1Id ? "win" : "loss") : undefined;
		const opp2Result =
			winnerId && p2Id ? (winnerId === p2Id ? "win" : "loss") : undefined;
