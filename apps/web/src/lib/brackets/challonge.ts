...
function setsToScore(
	sets: ScrapedMatch["sets"],
	side: 0 | 1,
): number | undefined {
...
export function challongeToViewerData(
	...
	const stage: Stage = {
		...
		settings: {
			size:
				tournament.metadata.participantsCount || tournament.participants.length,
			...
			opponent1:
				m.player1Id !== null
					? {
							...
							...(m.forfeited &&
								m.loserId === m.player1Id && { forfeit: true }),
						}
					: null,
			opponent2:
				m.player2Id !== null
					? {
							...
							...(m.forfeited &&
								m.loserId === m.player2Id && { forfeit: true }),
						}
					: null,
		};
	});
