...
function getWboTier(partName: string, partType: string): "S" | "A" | "B" | "C" {
	...
	const tiersMap =
		partType === "BLADE" ? WBO_BLADE_TIERS :
		partType === "RATCHET" ? WBO_RATCHET_TIERS :
		partType === "BIT" ? WBO_BIT_TIERS : {};
...
export async function getRecommendations(
	...
	for (const group of productGroups) {
		...
		if (partsList.length > 0) {
			...
			for (const part of partsList) {
				...
				const maxUsage =
					part.type === "BLADE" ? maxBladeCount :
					part.type === "RATCHET" ? maxRatchetCount :
					part.type === "BIT" ? maxBitCount : 1;
	...
	for (const sg of scoredGroups) {
		...
		if (maxRatio > minRatio) {
			priceEfficiencyScore =
				(sg.rawEfficiencyRatio - minRatio) / (maxRatio - minRatio);
		} else {
	...
	if (filters) {
		...
		if (filters.maxPriceEur !== undefined) {
			filtered = filtered.filter((r) => r.cheapestEur !== null && r.cheapestEur <= filters.maxPriceEur!);
		}
		if (filters.productType !== undefined) {
			filtered = filtered.filter((r) => {
				const dbType = dbProducts.find((p) => p.code.toUpperCase() === r.code?.toUpperCase())?.productType;
				return dbType?.toUpperCase() === filters.productType!.toUpperCase();
			});
		}
		if (filters.productLine !== undefined) {
			filtered = filtered.filter((r) => {
				const dbLine = dbProducts.find((p) => p.code.toUpperCase() === r.code?.toUpperCase())?.productLine;
