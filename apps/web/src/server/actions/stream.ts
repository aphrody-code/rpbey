...
export async function updateStreamState(updates: Record<string, unknown>) {
  try {
    ...
    if (updates.score) {
      const scoreUpdates = (updates.score as Record<string, unknown>)["1"] as Record<
        string,
        unknown
      >;
      if (scoreUpdates) {
        if (!state.score) state.score = { "1": {} };
        const s1 = (state.score as Record<string, unknown>)["1"] as Record<string, unknown>;
        ...
        if (scoreUpdates.team) {
          ...
          for (const [teamKey, teamData] of Object.entries(teamUpdates)) {
            (s1.team as Record<string, unknown>)[teamKey] = {
              ...((s1.team as Record<string, unknown>)[teamKey] as Record<string, unknown>),
            ...
            if (td.player) {
              ...
              for (const [pk, pv] of Object.entries(td.player as Record<string, unknown>)) {
                if (!existing) break;
                existing[pk] = {
...
export async function swapStreamPlayers() {
  ...
  if (!s1?.team) return { ok: false as const, error: "Pas de données de match" };
...
export async function getStreamPlayers(): Promise<{
  ...
  try {
    ...
    const players = (data as Array<{ gamerTag?: string; prefix?: string }>).map((p) =>
