...
async function loadAliasResolver(): Promise<{
	...
	try {
		const map =
			(await loadJsonSafe<Record<string, AliasMapEntry>>(
...
> {
	...
	> = {};
	try {
		const loaded = await loadJsonSafe<typeof map>(
			"data/exports/participants_map.json",
		);
	...
	for (const u of users)
		if (u.profile?.challongeUsername)
			setIfFree(norm(u.profile.challongeUsername), u.image);
	for (const u of users) {
...
export async function getBtsTournamentTop10(slug: string): Promise<{
	...
	try {
		...
		const data = await loadJsonSafe<BtsTournament>(
			`data/exports/B_TS${n}.json`,
		);
...
export async function getBtsSeasonTournaments(
	season: BtsSeason,
): Promise<BtsSeasonTournament[]> {
	const slugs = SEASON_MAP[season] ?? [];
	...
	const dbTournaments =
		challongeIds.length > 0
			...
			: [];
	const dbByChallongeId = new Map(
		dbTournaments.map((t) => [t.challongeId ?? "", t]),
	);
	...
	return exports_
		.map((e, i): BtsSeasonTournament | null => {
			...
			const db = challongeId
				? (dbByChallongeId.get(challongeId) ?? null)
				: null;
			return {
				slug,
