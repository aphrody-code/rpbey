...
import { ChallongeApi } from '@/lib/challonge-vendor';
import {
...
async function fetchViaHtmlRewriter(
	idOrSlug: string,
...
const fetchCachedAuto = cache(
	(idOrSlug: string): Promise<ChallongeImportResult> =>
		fetchViaTransport(idOrSlug, "auto"),
	["challonge-to-brackets-auto"],
	{ revalidate: 300, tags: [CACHE_TAG] },
);
const fetchCachedApi = cache(
	(idOrSlug: string): Promise<ChallongeImportResult> =>
		fetchViaTransport(idOrSlug, "api"),
	["challonge-to-brackets-api"],
	{ revalidate: 300, tags: [CACHE_TAG] },
);
const fetchCachedHtml = cache(
	(idOrSlug: string): Promise<ChallongeImportResult> =>
		fetchViaTransport(idOrSlug, "htmlrewriter"),
