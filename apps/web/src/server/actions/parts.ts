...
import { db, schema, and, or, eq, ne, inArray, ilike, asc, count, type SQL } from "@/lib/db";
...
export async function getPublicParts(params: {
  ...
  if (beyTypes && beyTypes.length > 0) {
    conditions.push(
      inArray(schema.parts.beyType, beyTypes as (typeof schema.beyType.enumValues)[number][]),
    );
  }
