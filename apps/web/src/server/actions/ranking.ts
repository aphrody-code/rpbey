...
export async function recalculateRankings() {
	...
	for (const d of enrichedData) {
		...
		playerStats.set(playerKey, {
			...
			userId: null,
			challongeUsername:
				d.challongeUsername !== "new" ? d.challongeUsername : null,
	...
	for (const tournament of dbTournaments) {
		const multiplier =
			tournament.tournamentCategory?.multiplier ?? tournament.weight ?? 1.0;
		for (const participant of tournament.tournamentParticipants) {
			...
			const stats = playerStats.get(playerKey) || {
				...
				challongeUsername:
					...
					participantProfile?.challongeUsername ||
					null,
	...
	for (const adj of adjustments) {
		...
		const baseKey = (userProfile?.bladerName || "unknown")
			.toLowerCase()
			.replace(/[^a-z0-9]/g, "");
			...
			if (ranking.userId) {
				await tx
					...
					.set({
						...
						...(ranking.challongeUsername
							? { challongeUsername: ranking.challongeUsername }
							: {}),
					})
...
export async function deleteTournamentCategory(id: string) {
	...
	await db
		.delete(schema.tournamentCategories)
		.where(eq(schema.tournamentCategories.id, id));
	// revalidateTag('tournament-categories');
	revalidatePath("/admin/rankings");
	return { success: true };
