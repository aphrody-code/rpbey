import "server-only";
import { and, asc, count, db, desc, eq, gt, inArray, or, schema } from "@/lib/db";
import { type LeaderboardEntry, type UserStats } from "@/lib/stats-types";

/**
 * Data Access Layer — statistiques joueur (ELO, leaderboard, head-to-head).
 * SEUL endroit autorisé à importer `@rpbey/db` pour ce domaine. UI-agnostic.
 *
 * Migré depuis `lib/stats.ts` (Phase 1.5) : `lib/stats.ts` reste une façade qui
 * ré-exporte d'ici, donc aucun appelant ne change mais le `db` quitte `lib/`.
 */
