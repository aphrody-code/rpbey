...
import { type StardustRanking } from "@/lib/types";
...
interface StardustComparatorProps {
  selected: StardustRanking[];
...
function CompareRadar({ a, b }: { a: StardustRanking; b: StardustRanking }) {
  ...
  const data = [
    ...
    { stat: "Moyenne", A: (avgA / maxAvg) * 100, B: (avgB / maxAvg) * 100 },
  ];
...
function CompareTable({ a, b }: { a: StardustRanking; b: StardustRanking }) {
  const wrA = parseNum(a.winRate);
  const wrB = parseNum(b.winRate);
  const avgA = parseNum(a.pointsAverage);
  const avgB = parseNum(b.pointsAverage);
...
/**
 * Comparator Stardust — calqué sur `SatrComparator`. Affiche une floating bar
 * tant qu'au moins un blader est sélectionné, et un Dialog (radar + tableau
 * comparatif + verdict) quand 2 sont sélectionnés.
 */
export function StardustComparator({ selected, onRemove, onClear }: StardustComparatorProps) {
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography
              variant="h6"
              sx={{ fontWeight: 900, fontSize: { xs: "1rem", sm: "1.25rem" } }}
            >
              Comparaison
            </Typography>
          </Box>
