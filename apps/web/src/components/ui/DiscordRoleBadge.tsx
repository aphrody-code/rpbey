...
import { getRoleTypeFromDiscordId, RoleColors, type RoleType } from "@/lib/role-colors";

const RoleIcon = dynamic(() => import("./RoleIcon").then((mod) => mod.RoleIcon), {
  ssr: false,
...
export const DiscordRoleBadge: React.FC<DiscordRoleBadgeProps> = ({
  ...
  // Determine Role Type
  const type = roleType || (roleId ? getRoleTypeFromDiscordId(roleId) : "DEFAULT");
    ...
    >
      <Box
        sx={{
          ...
          boxShadow: variant === "glow" ? `0 0 15px ${alpha(color, 0.3)}` : "none",
          color: color,
          userSelect: "none",
          cursor: "default",
