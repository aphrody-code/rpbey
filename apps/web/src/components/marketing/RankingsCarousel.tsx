...
export function RankingsCarousel({ boards }: { boards: RankingBoard[] }) {
  ...
  const paginate = useCallback((next: number, dir: number) => setState([next, dir]), []);

  const go = useCallback((dir: number) => paginate(safeIndex + dir, dir), [paginate, safeIndex]);
          ...
          {available.map((b, i) => (
            <Box
              ...
              sx={{
                ...
                p: 0,
                cursor: "pointer",
                bgcolor: i === safeIndex ? board.color : "rgba(255,255,255,0.18)",
