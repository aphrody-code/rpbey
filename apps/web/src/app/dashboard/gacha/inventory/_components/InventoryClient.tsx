...
import { Close, NavigateBefore, NavigateNext, Favorite, FavoriteBorder } from "@mui/icons-material";
import Image from "next/image";
import { useRouter, useSearchParams } from "next/navigation";
import { GachaInventoryCard, type InventoryItemShape } from "@/components/GachaInventoryCard";
...
export function InventoryClient({
  ...
  const [wishlistIds, setWishlistIds] = useState<Set<string>>(initialWishlistIds);
  ...
  const rarityValues: CardRarity[] = ["COMMON", "RARE", "SUPER_RARE", "LEGENDARY", "SECRET"];
      ...
      {(hasPrev || hasNext) && (
        <Stack direction="row" spacing={2} sx={{ mt: 4, justifyContent: "center" }}>
          <Button
            ...
            disabled={!hasPrev}
            onClick={() => router.push(buildUrl({ cursor: prevCursor, prev: "1" }))}
          >
            Precedent
          </Button>
          <Button
            ...
            disabled={!hasNext}
            onClick={() => router.push(buildUrl({ cursor: nextCursor, prev: null }))}
          >
            Suivant
          </Button>
        </Stack>
              ...
              <Grid container spacing={3}>
                ...
                <Grid size={{ xs: 12, sm: 7 }}>
                  <Stack spacing={1.5}>
                    ...
                    <Button
                      variant={wishlistIds.has(selected.card.id) ? "contained" : "outlined"}
                      startIcon={
                        wishlistIds.has(selected.card.id) ? <Favorite /> : <FavoriteBorder />
                      }
