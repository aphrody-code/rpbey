...
export default async function GachaInventoryPage({ searchParams }: PageProps) {
	...
	const rarity: CardRarity | "" = VALID_RARITIES.has(rawRarity)
		? (rawRarity as CardRarity)
		: "";
	...
	if (filterCardIds)
		baseConds.push(inArray(schema.cardInventory.cardId, filterCardIds));
	if (cursorObtainedAt) {
		baseConds.push(
			isBack
	...
	const rawItems = await db.query.cardInventory.findMany({
		...
		orderBy: isBack
			? asc(schema.cardInventory.obtainedAt)
			: desc(schema.cardInventory.obtainedAt),
		limit: PAGE_SIZE + 1,
	});
