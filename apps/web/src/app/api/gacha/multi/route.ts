...
export async function POST() {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (profile.currency < MULTI_PULL_COST) throw new Error("INSUFFICIENT_FUNDS");

      // Roll all rarities
      const rarities = Array.from({ length: MULTI_PULL_COUNT }, () => rollCardRarity());

      // Guarantee at least 1 SR+
      const hasSRPlus = rarities.some((r) => ["SUPER_RARE", "LEGENDARY", "SECRET"].includes(r));
      if (!hasSRPlus) {
