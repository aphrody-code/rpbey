...
export async function GET(request: NextRequest) {
	try {
		...
		if (!bladeId || !ratchetId || !bitId) {
			return new Response(
				JSON.stringify({ error: "blade, ratchet, and bit params required" }),
		...
		return new ImageResponse(
			...
			>
				...
				>
					<StatBar
						label="ATTAQUE"
						value={Number(blade.attack) || 0}
						color="#ef4444"
					/>
					<StatBar
						label="DÉFENSE"
						value={Number(blade.defense) || 0}
						color="#3b82f6"
					/>
					<StatBar
						label="ENDURANCE"
						value={Number(blade.stamina) || 0}
						color="#22c55e"
					/>
					<StatBar
						label="DASH"
						value={Number(blade.dash) || 0}
						color="#eab308"
					/>
				</div>
			</div>,
		...
		return new Response(
			JSON.stringify({ error: "Failed to generate combo card" }),
