...
export default function StreamConfigPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (stateRes.ok && stateRes.state) {
      ...
      if (s1) {
        ...
        if (p1?.name) setPlayer1(p1.prefix ? `${p1.prefix} | ${p1.name}` : p1.name);
        if (p2?.name) setPlayer2(p2.prefix ? `${p2.prefix} | ${p2.name}` : p2.name);
  ...
  const handleUpdateTournament = async () => {
    const type = phase.includes("Tamashii") ? "bts" : phase.includes("Ultime") ? "wb" : "standard";
    const res = await updateStreamState({
      tournament: {
        name: tournamentName,
        format: type === "bts" ? "3on3 Double Élimination" : "1v1 Double Élimination",
        organizer: type === "bts" ? "RPB" : type === "wb" ? "Wild Breakers" : "RPB",
      },
    });
    if (res.ok) {
  ...
  const handleScoreChange = async (team: "1" | "2", delta: number) => {
    const newScore = team === "1" ? Math.max(0, score1 + delta) : Math.max(0, score2 + delta);
        ...
        <Grid size={{ xs: 12, lg: 8 }}>
          ...
          >
            ...
            >
              ...
              >
                <Chip label={phase || "Phase"} color="primary" variant="outlined" />
            ...
            <Card variant="outlined" sx={{ borderRadius: 4 }}>
              <CardContent>
                ...
                <Stack spacing={2}>
                  ...
                  />
                  <Button variant="outlined" onClick={handleUpdateTournament} fullWidth>
                    Mettre à jour
                  </Button>
                </Stack>
              </CardContent>
