...
export default function TournamentDetail({
  ...
  const { url: posterUrl, unoptimized: unoptimizedPoster } = resolvePoster(tournament);

  const { liveData } = useLiveTournament(tournament.id, initialLiveData as LiveData, isLive);
  ...
  const liveCounters = isLive
    ? {
        completed: activityLog.filter((e) => (e.type ?? "").includes("match")).length,
    ...
    <Box sx={{ width: "100%", py: { xs: 2, md: 4 }, px: { xs: 2, md: 4, lg: 6 } }}>
      ...
      <Grid container spacing={{ xs: 3, md: 5 }}>
        ...
        <Grid size={{ xs: 12, lg: 8, xl: 9 }} sx={{ order: { xs: 2, lg: 1 } }}>
          ...
          <AboutSection tournament={tournament} isBTS={isBTS} />
          ...
          >
            ...
            >
              ...
              <Tab icon={<PoolsIcon sx={{ fontSize: 20 }} />} iconPosition="start" label="POULES" />
              <Tab
              ...
              {activeTab === 0 &&
                (mirrorHtml ? (
                  <MirrorReact
                    ...
                    baseUrl={
                      tournament.challongeUrl?.split("/module")[0] ?? "https://challonge.com"
                    }
                  />
                ) : (
                  <TournamentBracketDb
