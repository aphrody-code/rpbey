...
export function TournamentHeader({ name, status, isBTS, isLive, liveCounters }: Props) {
  return (
    <Box sx={{ mb: 4 }}>
      <Stack direction="row" spacing={1.5} sx={{ alignItems: "center", mb: 1 }}>
        <TournamentStatusChip status={(status || "").toLowerCase() as TournamentStatus} />
        {isBTS && (
          <Chip
            label="OFFICIEL RPB"
            size="small"
            sx={{
              ...
              fontSize: "0.65rem",
              letterSpacing: 1,
              border: (t) => `1px solid ${alpha(t.palette.secondary.main, 0.3)}`,
            }}
          />
        )}
        {isLive && (
