...
import { getAwardsLeaders, listPolls, listTierLists } from "@/server/dal/polls";
...
export default async function SondagesPage() {
  const [awards, polls, tierLists, awardsLeaders] = await Promise.all([
    ...
    getAwardsLeaders(AWARDS_CATEGORY),
  ]);
  ...
  return (
    <SondagesHub
      awards={awards.items}
      awardsLeaders={awardsLeaders}
      polls={generalPolls}
      tierLists={tierLists.items}
    />
  );
}
