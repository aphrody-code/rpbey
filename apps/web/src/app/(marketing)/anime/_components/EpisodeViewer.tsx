...
function getSourceLabel(source: Source): string {
  ...
  if (url.includes("youtube.com") || source.type === "YOUTUBE") return "YouTube";
...
export function EpisodeViewer({
  ...
  const langSources = streamingSources.filter((s) => s.language === selectedLang);

  const [activeIdx, setActiveIdx] = useState(0);
  const activeSource = langSources[activeIdx] || langSources[0];

  const hasControls = languages.length > 1 || langSources.length > 1 || downloadSources.length > 0;
  ...
  return (
    <Box>
      ...
      <EpisodePlayerWrapper
        ...
        episodeId={episodeId}
        duration={duration}
        startTime={seriesSlug === "beyblade-x" && selectedLang === "VOSTFR" ? 4 : 0}
        onEnded={onEnded}
      />
      ...
      {hasControls && (
        ...
        >
              ...
              {languages.map((lang) => (
                <Chip
                  ...
                  sx={{
                    ...
                    fontSize: "0.7rem",
                    bgcolor:
                      selectedLang === lang ? "var(--rpb-primary)" : "rgba(255,255,255,0.06)",
                    color: selectedLang === lang ? "white" : "rgba(255,255,255,0.5)",
                    border:
                      selectedLang === lang
                    ...
                    "&:hover": {
                      bgcolor:
                        selectedLang === lang ? "var(--rpb-primary)" : "rgba(255,255,255,0.1)",
                    },
                  }}
                />
              ))}
          ...
          {langSources.length > 1 && (
            ...
            >
              <OndemandVideo sx={{ color: "rgba(255,255,255,0.3)", fontSize: 16 }} />
              {langSources.map((src, i) => (
                <Chip
                  ...
                  sx={{
                    ...
                    fontWeight: 600,
                    fontSize: "0.7rem",
                    bgcolor: activeIdx === i ? "rgba(255,255,255,0.12)" : "transparent",
          ...
          {downloadSources.length > 0 && (
            <Tooltip title="Télécharger (Google Drive)" arrow>
              ...
              >
                <Box sx={{ display: { xs: "none", sm: "inline" } }}>Télécharger</Box>
              </Button>
            </Tooltip>
          )}
        </Box>
