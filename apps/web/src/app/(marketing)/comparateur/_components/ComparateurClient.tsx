...
export function ComparateurClient({ products, shops, groups, generatedAt, stats, recommendations = [] }: Props) {
	...
	const scoredRecommendations = useMemo(() => {
		...
		if (query) {
			list = list.filter((r) => {
				...
				const partsMatch = r.includedParts.some((p) =>
	...
	const recCols: GridColDef[] = [
			...
			renderCell: (p) => {
				...
				return (
					<Stack direction="row" spacing={1} sx={{ alignItems: "center", width: "100%", pr: 1 }}>
						...
						/>
						<Typography sx={{ fontWeight: 800, fontSize: "0.78rem", width: 35, textAlign: "right" }}>
							{Math.round(val)}%
						</Typography>
					</Stack>
				);
		...
		},
		{
			...
			headerName: "Code",
			width: 80,
			renderCell: (p) => (p.value ? <Chip size="small" label={p.value} sx={{ fontWeight: 800, fontSize: "0.65rem", bgcolor: "rgba(255,255,255,0.06)" }} /> : "—"),
		},
		{
					...
					<Stack direction="row" spacing={0.5} sx={{ flexWrap: "wrap", gap: 0.5 }}>
						{badges.slice(0, 2).map((b) => {
							...
							if (b === "Competitive Pick") {
								customStyle = { bgcolor: "rgba(168, 85, 247, 0.15)", color: "#c084fc", fontWeight: 800 };
							} else if (b === "Hype / New Release") {
								customStyle = { bgcolor: "rgba(244, 63, 94, 0.15)", color: "#fda4af", fontWeight: 800 };
							} else if (b === "Budget / Great Value") {
								customStyle = { bgcolor: "rgba(6, 182, 212, 0.15)", color: "#67e8f9", fontWeight: 800 };
							} else if (b === "Collector Choice") {
								customStyle = { bgcolor: "rgba(59, 130, 246, 0.15)", color: "#93c5fd", fontWeight: 800 };
							} else if (b === "Starter Pick") {
								customStyle = { bgcolor: "rgba(34, 197, 94, 0.15)", color: "#86efac", fontWeight: 800 };
							} else if (b === "Essential Accessory") {
								customStyle = { bgcolor: "rgba(255, 255, 255, 0.08)", color: "#d1d5db", fontWeight: 800 };
							}
		...
		if (query) {
			list = list.filter(
				(s) =>
					...
					normalizeText(s.domain).includes(query),
			);
		}
	...
	const groupCols: GridColDef[] = [
		...
		{ field: "shopCount", headerName: "Boutiques", width: 100, type: "number" },
		{
			...
			type: "number",
			renderCell: (p) => (
				<Typography sx={{ fontWeight: 900, color: "#22c55e", fontSize: "0.95rem" }}>{fmtEur(p.value as number)}</Typography>
			),
		},
		{
			field: "cheapest",
	...
	const productCols: GridColDef[] = [
		...
		},
		{
			...
			type: "number",
			renderCell: (p) => <Typography sx={{ fontWeight: 800, color: "#22c55e" }}>{fmtEur(p.value as number)}</Typography>,
		},
		{
			field: "price",
		...
		},
		{
			...
			renderCell: (p) => (
				<Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
					...
					<Typography sx={{ fontSize: "0.8rem", fontWeight: 700, color: p.value ? "success.main" : "warning.main" }}>
	...
	const shopCols: GridColDef[] = [
		...
		{ field: "type", headerName: "Type", width: 130, renderCell: (p) => TYPE_LABEL[p.value as string] ?? p.value },
		{
			...
			type: "number",
			renderCell: (p) => (
				<Typography sx={{ fontWeight: 800 }}>{p.value as number}</Typography>
			)
		},
	];
	...
	return (
		<Box>
							...
							{loadingGlobal ? (
								...
								<Stack spacing={2.5}>
									...
									{globalResults.product.length > 0 && (
										<Box>
											<Typography variant="subtitle2" sx={{ color: "text.secondary", fontWeight: 800, fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: 1, mb: 1, display: "flex", alignItems: "center", gap: 0.5 }}>
											...
											<Stack spacing={0.5}>
												{globalResults.product.slice(0, 4).map((item: any) => (
													...
													>
														...
														<Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
															{item.price && (
																<Typography sx={{ fontWeight: 800, color: "#22c55e", fontSize: "0.82rem" }}>
																	{fmtEur(item.price)}
																</Typography>
															)}
													...
													>
														...
														<Chip
															...
															sx={{
																...
																fontSize: "0.58rem",
																fontWeight: 900,
																bgcolor: item.badge?.includes("Tier S") ? "rgba(239, 68, 68, 0.15)" : item.badge?.includes("Tier A") ? "rgba(168, 85, 247, 0.15)" : "rgba(255, 255, 255, 0.08)",
																color: item.badge?.includes("Tier S") ? "#f87171" : item.badge?.includes("Tier A") ? "#c084fc" : "#d1d5db",
																border: "none",
															}}
														/>
													</Box>
												...
												{globalResults.tournament.slice(0, 3).map((item: any) => (
													...
													>
														...
														<Chip
															size="small"
															label={item.badge}
															color={item.badge === "COMPLETE" ? "success" : item.badge === "UNDERWAY" ? "warning" : "primary"}
															variant="outlined"
			...
			<Stack direction="row" spacing={3} sx={{ alignItems: "flex-start", position: "relative", width: "100%" }}>
				...
				<Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
					...
					<Box sx={{
						...
						<Typography variant="body2" sx={{ color: "text.secondary", mb: 3, lineHeight: 1.5 }}>
							Personnalisez les coefficients de pondération ci-dessous pour ajuster le score d'évaluation des produits. Le classement s'actualise en temps réel.
						</Typography>
						...
						<Box sx={{
							...
							<Box>
								<Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 1 }}>
									<Typography sx={{ fontWeight: 800, fontSize: "0.85rem", display: "flex", alignItems: "center", gap: 0.5 }}>
								...
								<Typography variant="caption" sx={{ color: "text.secondary", display: "block", mt: 0.5, lineHeight: 1.4 }}>
									Coefficients des pièces basés sur l'usage en tournoi (decks enregistrés) et la classification WBO.
								</Typography>
							</Box>
							...
							<Box>
								...
								/>
								<Typography variant="caption" sx={{ color: "text.secondary", display: "block", mt: 0.5, lineHeight: 1.4 }}>
									Popularité en magasin, fraîcheur de sortie (&lt;1 an) et statut d'édition Collector/Limitée.
								</Typography>
							</Box>
							...
							<Box>
								<Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 1 }}>
									<Typography sx={{ fontWeight: 800, fontSize: "0.85rem", display: "flex", alignItems: "center", gap: 0.5 }}>
										<AccountBalanceWallet sx={{ fontSize: 16, color: "#06b6d4" }} /> Rapport Qualité/Prix
									</Typography>
								...
								/>
								<Typography variant="caption" sx={{ color: "text.secondary", display: "block", mt: 0.5, lineHeight: 1.4 }}>
									Rapport entre la valeur estimée (méta + hype) et le meilleur tarif constaté sur le marché.
								</Typography>
							</Box>
						</Box>
					</Box>
				...
				<Box sx={{ display: "flex", flexDirection: "column", gap: 3.5 }}>
					...
					<Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2.5 }}>
						<Box sx={{
							...
							<Typography variant="subtitle2" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: 0.8, fontWeight: 800, fontSize: "0.65rem", mb: 1 }}>
								Prix Moyen Global
							</Typography>
							<Typography variant="h4" sx={{ fontWeight: 900, color: "var(--rpb-primary)" }}>
								{fmtEur(stats.averagePriceEur)}
							</Typography>
							<Typography variant="caption" sx={{ color: "text.secondary", mt: 1, display: "block", lineHeight: 1.4 }}>
								Calculé à partir de toutes les offres indexées avec un prix valide converti en euros.
							</Typography>
						</Box>
						...
						<Box sx={{
							...
							<Typography variant="caption" sx={{ color: "text.secondary", mt: 1, display: "block", lineHeight: 1.4 }}>
								{shops.filter(s => s.productCount > 0).length} boutiques scrapées avec succès sur un total de {shops.length}.
							</Typography>
						</Box>
					</Box>
						...
						<Box sx={{
							...
							<Stack spacing={2}>
								{stats.regionStats.map((r) => (
									<Box key={r.region} sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1.5, borderBottom: "1px solid rgba(255,255,255,0.05)", "&:last-child": { pb: 0, border: 0 } }}>
										<Box>
						...
						<Box sx={{
							...
							<Stack spacing={2}>
								{stats.platformStats.map((p) => (
									<Box key={p.platform} sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1.5, borderBottom: "1px solid rgba(255,255,255,0.05)", "&:last-child": { pb: 0, border: 0 } }}>
										<Box>
											<Typography sx={{ fontWeight: 700, textTransform: "capitalize", fontSize: "0.95rem" }}>
												{p.platform}
											</Typography>
											<Typography variant="caption" sx={{ color: "text.secondary" }}>
										...
										<Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
											<Chip
												...
												variant="outlined"
												sx={{ fontWeight: 800, fontSize: "0.65rem", bgcolor: p.active > 0 ? "rgba(34,197,94,0.05)" : "transparent" }}
											/>
										</Box>
									</Box>
								))}
			...
			<Typography variant="caption" sx={{ color: "text.secondary", display: "block", mt: 2.5, opacity: 0.6 }}>
				...
				<AnimatePresence>
					{isDesktop && openGroup && (tab === 0 || tab === 1) && (
						...
						>
							<Box sx={{ pb: 2, mb: 2, borderBottom: "1px solid rgba(255,255,255,0.05)", position: "relative", pr: 5 }}>
								<Typography variant="h6" sx={{ fontWeight: 900, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 1.5, lineHeight: 1.2 }}>
									{openGroup.name}
									{openGroup.code && <Chip size="small" label={openGroup.code} sx={{ fontWeight: 800, fontSize: "0.65rem", bgcolor: "rgba(255,255,255,0.08)" }} />}
								</Typography>
								<IconButton
									aria-label="close"
							...
							<Typography variant="subtitle2" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: 0.8, fontWeight: 800, fontSize: "0.68rem", mb: 1.5 }}>
								Offres en boutique ({openGroup.offers.length})
							</Typography>
							<Stack spacing={1.5} sx={{ mb: activeRec && activeRec.includedParts.length > 0 ? 4 : 0 }}>
									...
									>
										<Box sx={{ minWidth: 0, mr: 2 }}>
											<Stack direction="row" spacing={1} sx={{ alignItems: "center", mb: 0.5 }}>
												...
												>
													{REGION_FLAG[o.region] ?? ""} {o.shop} <OpenInNew sx={{ fontSize: 12, opacity: 0.7 }} />
												</MuiLink>
												{i === 0 && (
													<Chip
														size="small"
										...
										<Box sx={{ textAlign: "right", flexShrink: 0 }}>
											<Typography sx={{ fontWeight: 900, color: i === 0 ? "#22c55e" : "text.primary", fontSize: "1rem" }}>
												{fmtEur(o.priceEur)}
											</Typography>
											{o.currency !== "EUR" && (
							...
							{activeRec && activeRec.includedParts.length > 0 && (
								<Box sx={{ borderTop: "1px solid rgba(255,255,255,0.06)", pt: 3 }}>
									<Typography variant="subtitle2" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: 0.8, fontWeight: 800, fontSize: "0.68rem", mb: 2, display: "flex", alignItems: "center", gap: 0.8 }}>
									...
									<Stack spacing={1.5}>
										{activeRec.includedParts.map((part) => (
											...
											>
												<Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 1 }}>
													<Box>
														<Typography sx={{ fontWeight: 850, fontSize: "0.85rem", lineHeight: 1.2 }}>
															{part.name}
														</Typography>
														<Typography variant="caption" sx={{ color: "text.secondary", textTransform: "capitalize", fontSize: "0.65rem" }}>
															{part.type}
														</Typography>
													</Box>
													<Chip
														...
														sx={{
															...
															fontWeight: 900,
															bgcolor:
																part.tier === "S" ? "rgba(239, 68, 68, 0.15)" :
																part.tier === "A" ? "rgba(168, 85, 247, 0.15)" :
																part.tier === "B" ? "rgba(59, 130, 246, 0.15)" : "rgba(255, 255, 255, 0.08)",
															color:
																part.tier === "S" ? "#f87171" :
																part.tier === "A" ? "#c084fc" :
																part.tier === "B" ? "#60a5fa" : "#d1d5db",
															border: "none",
														}}
													/>
												</Stack>
			...
			>
				<DialogTitle sx={{ m: 0, p: 3, pr: 7, fontWeight: 900, display: "flex", alignItems: "center", gap: 1.5, borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
					{openGroup?.name}
					{openGroup?.code && <Chip size="small" label={openGroup.code} sx={{ fontWeight: 800, fontSize: "0.65rem", bgcolor: "rgba(255,255,255,0.08)" }} />}
				</DialogTitle>
				<IconButton
				...
				<DialogContent sx={{ p: 3 }}>
					...
					<Stack spacing={1.5} sx={{ mb: openGroup && (recommendations.find(r => r.key === openGroup.key)?.includedParts.length ?? 0) > 0 ? 4 : 0 }}>
						{openGroup?.offers.map((o, i) => (
							<Box
								component={motion.div}
								initial={{ opacity: 0, y: 12 }}
					...
					{openGroup && (recommendations.find(r => r.key === openGroup.key)?.includedParts.length ?? 0) > 0 && (
						<Box sx={{ borderTop: "1px solid rgba(255,255,255,0.06)", pt: 3 }}>
							<Typography variant="subtitle2" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: 0.8, fontWeight: 800, fontSize: "0.68rem", mb: 2, display: "flex", alignItems: "center", gap: 0.8 }}>
								<Shield sx={{ fontSize: 14, color: "#a855f7" }} /> Méta-Analyse des Composants
							</Typography>
							<Stack spacing={1.5}>
								{(recommendations.find(r => r.key === openGroup.key)?.includedParts ?? []).map((part) => (
									<Box
										key={part.id}
										sx={{
											p: 1.5,
