...
export type UpsellSurfaceKey = "UserProfileName" | "UserProfileHeader" | "HomeSidebar" | "PremiumNav" | "HomeNav" | string;
/** Checkout SKUs in responsive-web main bundle (2026-06); not separate GraphQL ops. */
export type ProductCategory = "BlueVerified" | "BlueVerified3Months" | "BlueVerified6Months" | "BlueVerifiedPlus" | "BlueVerifiedPlus3Months" | "BlueVerifiedPlus6Months" | "PremiumBasic" | string;
...
export declare function parsePremiumFlags(viewerJson: unknown, userJson?: unknown): PremiumAccountFlags;
export declare function fetchPremiumUpsells(client: XClient): Promise<PremiumUpsells>;
export declare function fetchPremiumBundle(client: XClient, handle: string, userId: string): Promise<{
...
export declare function premiumGraphqlVariables(handle: string, userId: string): Record<string, Record<string, unknown>>;
/** Fetch every op in PREMIUM_GRAPHQL_OPS; failures are recorded, not thrown. */
export declare function fetchAllPremiumGraphql(client: XClient, handle: string, userId: string): Promise<Record<string, {
    ok: true;
    data: unknown;
} | {
    ok: false;
