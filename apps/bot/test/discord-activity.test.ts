  ...
  return {
    ...
    profile: {
      upsert: async (args: {
        ...
        const existing = dbState.profiles.find((p) => p.userId === args.where.userId);
        if (existing) {
...
async function signPayload(timestamp: string, body: string): Promise<string> {
  ...
  const sig = await crypto.subtle.sign({ name: "Ed25519" } as never, PRIVATE_KEY, msg);
...
describe("verifyDiscordSignature", () => {
  it("returns true for a valid Ed25519 signature", async () => {
    ...
    expect(await verifyDiscordSignature(body, sig, ts, PUBLIC_KEY_HEX)).toBe(true);
  });

  it("returns false for an invalid signature", async () => {
    const ts = nowSeconds();
    const fakeSig = "00".repeat(64);
    expect(await verifyDiscordSignature("{}", fakeSig, ts, PUBLIC_KEY_HEX)).toBe(false);
  });

  it("returns false (does not throw) on malformed inputs", async () => {
    expect(await verifyDiscordSignature("body", "not-hex", "0", PUBLIC_KEY_HEX)).toBe(false);
  });
});
...
describe("handleEntitlementWebhook", () => {
  ...
  it("timestamp drift > 300s → throws INVALID_SIGNATURE", async () => {
    ...
    expect((caught as InstanceType<typeof WebhookError>).code).toBe("INVALID_SIGNATURE");
  ...
  it("invalid JSON body → throws INVALID_PAYLOAD", async () => {
    ...
    expect(caught).toBeInstanceOf(WebhookError);
    expect((caught as InstanceType<typeof WebhookError>).code).toBe("INVALID_PAYLOAD");
  });
  ...
  it("known SKU + missing user → throws USER_NOT_FOUND", async () => {
    ...
    expect(caught).toBeInstanceOf(WebhookError);
    expect((caught as InstanceType<typeof WebhookError>).code).toBe("USER_NOT_FOUND");
  });
  ...
  it("missing DISCORD_PUBLIC_KEY → throws MISSING_PUBLIC_KEY", async () => {
    ...
    expect(caught).toBeInstanceOf(WebhookError);
    expect((caught as InstanceType<typeof WebhookError>).code).toBe("MISSING_PUBLIC_KEY");
...
describe("exchangeDiscordCode", () => {
  ...
  it("happy path: exchanges code → token → user → mints session", async () => {
    ...
    expect(mintedSessions).toEqual([{ discordId: "discord-42", displayName: "Forty Two" }]);
  ...
  it("missing env → throws MISSING_ENV", async () => {
    ...
    expect((caught as InstanceType<typeof TokenExchangeError>).code).toBe("MISSING_ENV");
  });
  ...
  it("Discord 400 → throws INVALID_CODE", async () => {
    ...
    expect(caught).toBeInstanceOf(TokenExchangeError);
    expect((caught as InstanceType<typeof TokenExchangeError>).code).toBe("INVALID_CODE");
  });
  ...
  it("Discord 500 on token exchange → throws OAUTH_FAILED", async () => {
    ...
    expect(caught).toBeInstanceOf(TokenExchangeError);
    expect((caught as InstanceType<typeof TokenExchangeError>).code).toBe("OAUTH_FAILED");
  });
  ...
  it("user fetch fail → throws USER_FETCH_FAILED", async () => {
    ...
    expect(caught).toBeInstanceOf(TokenExchangeError);
    expect((caught as InstanceType<typeof TokenExchangeError>).code).toBe("USER_FETCH_FAILED");
  });
