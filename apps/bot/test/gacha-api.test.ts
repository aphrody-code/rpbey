...
interface PoolStub {
  ...
  handlers: Array<{
    ...
    respond: (sql: string, params: unknown[]) => Promise<{ rows: unknown[] }> | { rows: unknown[] };
  }>;
...
describe("ensureGachaSession — cache + reuse", () => {
  ...
  it("findReusableSession with ≥30 min remaining → reuses existing token, NO INSERT", async () => {
    ...
    const insertSessionCalls = pool.calls.filter((c) => /INSERT INTO sessions/i.test(c.sql));
  ...
  it("5 concurrent calls coalesce into a single mint (in-flight Promise)", async () => {
    ...
    const promises = Array.from({ length: 5 }, () => ensureGachaSession("discord-4", "Dave"));
