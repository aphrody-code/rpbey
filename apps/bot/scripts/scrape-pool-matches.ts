...
function parseMatchReport(text: string): {
	...
	if (!winner || !loser || winner.toLowerCase() === loser.toLowerCase())
		return null;
	return {
		winner,
		loser,
...
function extractLogEntries(store: Record<string, unknown>): LogEntry[] {
	...
	return rawEntries.map((e) => {
		...
		return {
			...
			text: String(
				entry["message"] ?? entry["description"] ?? entry["text"] ?? "",
			),
		};
	});
}
...
async function main() {
	...
	const afss = firstResult.store["ActivityFeedSettingsStore"] as Record<
		string,
		unknown
	> | null;
	const totalPages = Math.min(
		Number(afss?.["totalPages"] ?? afss?.["total_pages"] ?? 1),
		maxPages,
	);
	...
	const poolMatches = allMatches.filter(
		(m) => !bracketPairs.has(pairKey(m.winner, m.loser)),
	);
	console.log(
		`   bracket deja present: ${allMatches.length - poolMatches.length}`,
	);
