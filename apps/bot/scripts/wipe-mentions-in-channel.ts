...
const channelName = "name" in channel && channel.name ? channel.name : CHANNEL_ID;
const cutoffLabel = CUTOFF_MS ? `since ${new Date(CUTOFF_MS).toISOString()}` : "no cutoff";
log(
...
if (DRY) {
  ...
  if (matches.length > 0) {
    ...
    for (const m of matches.slice(0, 5)) {
      const snippet = (m.content || m.embeds[0]?.description || m.embeds[0]?.title || "[no text]")
        .replace(/\s+/g, " ")
        .slice(0, 120);
...
for (let i = 0; i < bulkable.length; i += 100) {
  ...
  if (slice.length < 2 || !("bulkDelete" in channel) || typeof channel.bulkDelete !== "function") {
