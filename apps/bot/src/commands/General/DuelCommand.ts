...
// Aliases from battle-engine (all constants preserved)
const RARITY_EMOJI = TCG_RARITY_EMOJI;
const RARITY_LABEL = TCG_RARITY_LABEL;
const ELEMENT_EMOJI = TCG_ELEMENT_EMOJI;
...
// Aliases for local use (all functions from battle-engine)
const hasSynergy = tcgHasSynergy;
const teamPower = tcgTeamPower;
const resolveRound = tcgResolveRound;
const getFinish = tcgGetFinish;
const getRankTier = tcgGetRankTier;
const cardSortPower = tcgCardSortPower;
const randomPick = pickRandom;
const powerBar = tcgPowerBar;
...
// ═══════════════════════════════════════════════════════════════════════════════
// TYPES (local aliases for battle-engine types)
// ═══════════════════════════════════════════════════════════════════════════════

type DuelCard = TcgDuelCard;
type RoundResult = TcgRoundResult;
...
export class DuelCommand {
  ...
  private async runDuel(
    ...
    const summaryContent =
      ...
      streakLine +
      mvpLine +
      `\n\n*Match #${match.id.slice(-8)} · ${RPB.Name}*`;

    // V2 text-only summary (canvas result card already sent via editReply above)
    const v2Result = v2Container({
      accentColor: finish.color,
      sections: [{ content: summaryContent, button: revengeBtn }],
    });

    // V2 messages: no embeds/content alongside components
    await interaction.followUp({ ...v2Result });
  }
