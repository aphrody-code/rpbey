...
export class EconomyGroup {
  ...
  async balance(interaction: CommandInteraction) {
    ...
    try {
      ...
      const totalCards = await this.prisma.gachaCard
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "🪙 Pièces",
            value: `**${bal.currency.toLocaleString("fr-FR")}** 🪙`,
            inline: true,
          },
          {
            name: "🔥 Streak",
            value: `**${bal.dailyStreak}** jour${bal.dailyStreak > 1 ? "s" : ""}`,
            inline: true,
          },
          {
    ...
    try {
      ...
      if (!result.card || !result.rarity) {
        ...
        const missEmbed = new EmbedBuilder()
          ...
          .setTitle("💨 Raté !")
          .setDescription(
            `La toupie s'est éjectée du stadium...\n\n💰 Solde : **${bal.toLocaleString("fr-FR")}** 🪙`,
          )
      ...
      const fullTitle = result.isWished ? `⭐ ${title}` : title;
      const cardDesc =
      ...
      if (cardAttachment) {
        // V2 panel — normalise filename to 'card.png' so MediaGallery URL matches
        const normalised = new AttachmentBuilder(
          (cardAttachment as AttachmentBuilder).attachment as Buffer,
          { name: "card.png" },
        );
        const v2 = v2GachaPullPanel(
          fullTitle,
          cardDesc,
          "card.png",
          result.isWished ? 0xfbbf24 : cfg.color,
        );
        if (v2) {
          reply = await interaction.editReply({ ...v2, files: [normalised] });
        } else {
          // v2GachaPullPanel only returns null when filename is undefined — won't happen here, but guard anyway
          const embed = new EmbedBuilder()
            .setColor(result.isWished ? 0xfbbf24 : cfg.color)
            .setTitle(fullTitle)
            .setDescription(cardDesc);
          reply = await interaction.editReply({ embeds: [embed], files: [cardAttachment] });
        }
      } else {
        const embed = new EmbedBuilder()
          .setColor(result.isWished ? 0xfbbf24 : cfg.color)
      ...
      if (result.badgeUnlocked) {
        ...
        await reply
          .reply({ content: `${b.emoji} **Badge "${b.name}" débloqué !** +${b.reward} 🪙` })
          .catch(() => null);
      }
    ...
    try {
      ...
      if (cards.length === 0) {
        return interaction.reply({
          embeds: [
            errorEmbed("Introuvable", `Carte "${cardName}" introuvable. \`/gacha catalogue\``),
          ],
      ...
      if (!r.added) {
        return interaction.reply({
          embeds: [warningEmbed("Wishlist", `**${card.name}** retirée de ta wishlist.`)],
        });
      }
      return interaction.reply({
        embeds: [
      ...
      if (!(err instanceof GachaApiError))
        console.error(`[gacha:${interaction.commandName}] unhandled`, err);
      if (err instanceof GachaApiError)
        return interaction.reply({ embeds: [gachaErrorEmbed(err)], flags: MessageFlags.Ephemeral });
      return interaction.reply({ embeds: [serviceDownEmbed()], flags: MessageFlags.Ephemeral });
    }
  }
  ...
  async wishlist(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (!(err instanceof GachaApiError))
        console.error(`[gacha:${interaction.commandName}] unhandled`, err);
      if (err instanceof GachaApiError)
        return interaction.reply({ embeds: [gachaErrorEmbed(err)] });
      return interaction.reply({ embeds: [serviceDownEmbed()] });
    }
  }
  ...
  async catalogue(
    ...
    try {
      ...
      if (filteredCards.length === 0) {
        return interaction.editReply({
          embeds: [warningEmbed("Catalogue", "Aucune carte trouvée.")],
        });
      }
  ...
  async collection(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (inv.items.length === 0) {
        return interaction.editReply({
          embeds: [infoEmbed("Collection vide", "`/gacha gacha` pour commencer !")],
        });
      }
  ...
  async sell(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (!dup) {
        return interaction.editReply({
          embeds: [warningEmbed("Aucun doublon", "Aucun doublon à vendre.")],
        });
      }
  ...
  async sellAll(interaction: CommandInteraction) {
    ...
    try {
      ...
      return interaction.editReply({
        embeds: [
          successEmbed(
            `${result.soldCount} doublons vendus !`,
  ...
  async leaderboard(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (collectors.length === 0) {
        return interaction.editReply({
          embeds: [infoEmbed("Classement vide", "Personne n'a de cartes !")],
        });
      }
      ...
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              {
                name: "🃏 Meilleurs Collectionneurs",
                value: collectorsLines.join("\n"),
                inline: true,
              },
    ...
    try {
      ...
      if (!targetDbUser) {
        ...
        await this.prisma.profile
          .create({ data: { userId: newUser.id, currency: 0 } })
          .catch(() => null);
      }

      // Use admin grant via gacha API (uses the caller's admin session)
      const targetFinal =
        targetDbUser ?? (await this.prisma.user.findUnique({ where: { discordId: target.id } }));
      if (!targetFinal) {
        return interaction.reply({
          content: "❌ Impossible de créer le compte.",
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      const r = await api.adminGrant(
        targetFinal.id,
        amount,
        reason ?? `Par ${interaction.user.displayName}`,
      );
  ...
  async duel(
    ...
    if (invA.length === 0) {
      return interaction.editReply({
        embeds: [errorEmbed("Aucune carte", "Tu n'as aucune carte ! `/gacha gacha`")],
      });
    }
    ...
    if (invB.length === 0) {
      return interaction.editReply({
        embeds: [errorEmbed("Aucune carte", `**${target.displayName}** n'a aucune carte !`)],
      });
    }
    ...
    if (!profile || profile.currency >= 0) {
      ...
      return interaction.reply({
        embeds: [
          successEmbed(
            "Aucune dette !",
            `Ton solde est de **${balance.toLocaleString("fr-FR")}** 🪙\nTu n'as aucune dette. Continue comme ça !`,
          ),
        ],
      });
    }
  ...
  async giveCurrency(
    ...
    return interaction.editReply({
      embeds: [
        successEmbed(
          "Don effectué !",
          `**${amount.toLocaleString("fr-FR")}** 🪙 envoyés à **${target.displayName}** !\n\n💰 Ton solde : **${(profile.currency - amount).toLocaleString("fr-FR")}** 🪙\n💰 Solde de ${target.displayName} : **${(targetProfile.currency + amount).toLocaleString("fr-FR")}** 🪙`,
        ),
      ],
    });
  }
