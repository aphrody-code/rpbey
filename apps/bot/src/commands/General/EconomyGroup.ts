...
import { errorEmbed, infoEmbed, successEmbed, v2GachaPullPanel, warningEmbed } from '../../lib/ui.js';
...
export class EconomyGroup {
  ...
  async gacha(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (cardAttachment) {
        ...
        const v2 = v2GachaPullPanel(fullTitle, cardDesc, 'card.png', result.isWished ? 0xfbbf24 : cfg.color);
        if (v2) {
    ...
    try {
      ...
      if (cards.length === 0) {
        return interaction.reply({
          embeds: [errorEmbed('Introuvable', `Carte "${cardName}" introuvable. \`/gacha catalogue\``)],
  ...
  async wishlist(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (items.length === 0) {
        return interaction.reply({
          embeds: [infoEmbed('Wishlist vide', 'Ajoute des cartes avec `/gacha wish <nom>`\nTu seras notifié par un embed doré quand tu les obtiendras !')],
        });
      }
  ...
  async debt(interaction: CommandInteraction) {
    ...
    if (!profile || profile.currency >= 0) {
      ...
      return interaction.reply({
        embeds: [
          successEmbed('Aucune dette !', `Ton solde est de **${balance.toLocaleString('fr-FR')}** 🪙\nTu n'as aucune dette. Continue comme ça !`),
        ],
      });
    }
  ...
  async giveCurrency(
    ...
    if (profile.currency < amount) {
      return interaction.editReply({
        embeds: [errorEmbed('Fonds insuffisants', `Tu n'as que **${profile.currency.toLocaleString('fr-FR')}** 🪙`)],
      });
    }
  ...
  async tradeCard(
    ...
    try {
      ...
      if (!targetDbUser) {
        return interaction.editReply({
          embeds: [errorEmbed('Compte introuvable', `**${target.displayName}** n'a pas de compte gacha.`)],
        });
      }
  ...
  async dropInfo(interaction: CommandInteraction) {
    ...
    try {
      ...
      if (!activeBanner) {
        return interaction.reply({
          embeds: [warningEmbed('Aucun drop actif', "Aucun drop de cartes n'est actif actuellement. Reviens plus tard !")],
        });
      }
