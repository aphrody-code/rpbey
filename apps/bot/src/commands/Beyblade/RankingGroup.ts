...
export class RankingGroup {
  ...
  async profile(
    ...
    try {
      ...
      const deckData = activeDeck?.items.some((i: any) => i.blade)
        ? {
      ...
      // Cache the canvas render (expensive, 1h TTL)
      const cardBuffer = await cachedBuffer(
        `card:profile:${target.id}:${rankingPoints}:${user._count.tournaments}`,
        TTL.CARD,
        () =>
          generateProfileCard({
            activeDeck: deckData,
      ...
      const winRateStr = this.computeWinRate(user.profile.wins, user.profile.losses);
      const v2 = v2ProfilePanel(
        user.profile.bladerName || target.displayName,
        rankTitle,
        rank,
        rankingPoints,
        user.profile.wins,
