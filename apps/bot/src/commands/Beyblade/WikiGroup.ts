...
export class WikiGroup {
  ...
  async rules(
    ...
    try {
      ...
      if (!(await file.exists())) return interaction.editReply("❌ Base de données indisponible.");
      const data = await file.json();
      const paragraphs = (data.translatedMarkdown || data.markdown).split("\n");
      const match = paragraphs.find((p: string) => p.toLowerCase().includes(query.toLowerCase()));
      if (!match) return interaction.editReply(`❌ Aucun résultat pour "${query}".`);

      const embed = new EmbedBuilder()
        .setTitle(`📖 Wiki : ${query}`)
        .setDescription(match.length > 1000 ? `${match.substring(0, 1000)}...` : match)
