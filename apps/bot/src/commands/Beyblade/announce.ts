...
export class AnnounceCommand {
  ...
  async tournament(
    ...
    const channel = channelOption ?? (interaction.channel as TextChannel | NewsChannel);
    ...
    try {
      ...
      const mention = mentionRole ? `${mentionRole}` : `<@&${RPB.Roles.TournoiNotification}>`;
  ...
  async results(
    ...
    try {
      ...
      const podiumText = podium.map((p, i) => `${medals[i]} **${p.attributes.name}**`).join("\n");
  ...
  async custom(
    ...
    const announcement = [mention ? mention.trim() : null, `# 📢 ${title}`, ``, message]
