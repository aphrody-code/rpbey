...
import { type ScrapedStanding, type ScrapedStation } from "../../lib/scrapers/challonge-scraper.js";
...
export class TournamentLiveCommand {
  ...
  async live(
    @SlashOption({
      name: "tournoi",
      description: "Nom ou ID Challonge du tournoi (par défaut: premier tournoi en cours)",
      required: false,
      type: ApplicationCommandOptionType.String,
    })
    ...
    try {
      const tournament = search
        ? await prisma.tournament.findFirst({
            where: {
              OR: [{ name: { contains: search, mode: "insensitive" } }, { challongeId: search }],
              status: "UNDERWAY",
            },
          })
        : await prisma.tournament.findFirst({
      ...
      const standings = (tournament.standings as unknown as ScrapedStanding[] | null) || [];
      const stations = (tournament.stations as unknown as ScrapedStation[] | null) || [];
      ...
      const standingsText =
        top5.length > 0
          ? top5.map((s) => `**${s.rank}.** ${s.name} (${s.wins}V - ${s.losses}D)`).join("\n")
