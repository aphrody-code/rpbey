...
export class ReadyListener {
  ...
  async onReady([client]: ArgsOf<"clientReady">) {
    ...
    if (process.env.CLEAR_GUILD_COMMANDS === "true" && process.env.GUILD_ID) {
      try {
        logger.info(`Nettoyage des commandes de guilde pour : ${process.env.GUILD_ID}`);
        const guild = await client.guilds.fetch(process.env.GUILD_ID);
        await guild.commands.set([]);
        logger.info("Commandes de guilde nettoyées avec succès !");
      } catch (error) {
        logger.error("Erreur lors du nettoyage des commandes de guilde :", error);
      }
    }
  }
  ...
  private async syncAdminRoles(client: Client) {
    ...
    if (!guildId) {
      logger.warn("GUILD_ID pas configuré, impossible de synchroniser les rôles admin.");
      return;
    }
    ...
    try {
      ...
      if (!guild) {
        logger.error(`Guilde ${guildId} non trouvée lors de la synchronisation des rôles.`);
        return;
      }
