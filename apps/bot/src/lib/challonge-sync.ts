...
export async function scrapeAndSyncTournament(urlId: string): Promise<SyncResult> {
  ...
  try {
    ...
    const dbTournament = await prisma.tournament.upsert({
      ...
      update: {
        ...
        standings: result.standings.length > 0 ? (result.standings as never) : undefined,
        stations: result.stations.length > 0 ? (result.stations as never) : undefined,
        activityLog: result.log.length > 0 ? (result.log as never) : undefined,
      },
      create: {
        challongeId,
        name: meta.name,
...
export async function shouldSyncRealtime(tournamentId: string): Promise<boolean> {
  ...
  const hoursUntilTournament = (tournamentDate.getTime() - now.getTime()) / (1000 * 60 * 60);

  return (
    hoursUntilTournament <= 24 && hoursUntilTournament > -6 && tournament.status !== "COMPLETE"
  );
}
...
export async function getTournamentsNeedingSync(): Promise<string[]> {
  ...
  return tournaments.map((t: any) => t.challongeId).filter((id: any): id is string => id !== null);
}
...
> {
  ...
  return tournaments.filter(
    (t: any): t is typeof t & { challongeId: string } => t.challongeId !== null,
  );
}
