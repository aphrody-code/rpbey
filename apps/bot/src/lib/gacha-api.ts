...
async function upsertUser(discordUserId: string, displayName: string): Promise<string> {
  const pool = getPool();
  ...
  if (!row) {
    throw new Error(`upsertUser: no row returned for discordId=${discordUserId}`);
  }
  return row.id;
}
...
async function findReusableSession(userId: string): Promise<CachedSession | null> {
  const pool = getPool();
...
async function mintSession(discordUserId: string, displayName: string): Promise<CachedSession> {
...
async function getSession(discordUserId: string, displayName: string): Promise<CachedSession> {
...
export interface GachaApiClient {
  ...
  inventory(opts?: { rarity?: string; cursor?: string; limit?: number }): Promise<InventoryPage>;
  ...
  history(opts?: { cursor?: string; limit?: number; type?: string }): Promise<HistoryPage>;
...
async function apiFetch<T>(
  ...
  try {
    ...
    if ((err as Error).name === "AbortError") {
      throw new GachaApiError("TIMEOUT", "Le serveur gacha ne répond pas (timeout 15s)");
    }
    throw new GachaApiError("NETWORK", `Erreur réseau gacha : ${(err as Error).message}`);
  } finally {
  ...
  if (!response.ok || json.ok === false) {
    const err = json.error as { code?: string; message?: string; retryInMs?: number } | undefined;
    const flatCode = typeof json.code === "string" ? json.code : undefined;
    const flatMessage = typeof json.message === "string" ? json.message : undefined;
    throw new GachaApiError(
...
export async function createGachaClient(
  ...
  const get = <T>(path: string, q?: Record<string, string | number | undefined>) =>
    apiFetch<T>(t, "GET", path, undefined, q);
  const post = <T>(path: string, body?: unknown) => apiFetch<T>(t, "POST", path, body);

  return {
    // ── Gacha ──
    async pull() {
      const r = await post<{ ok: boolean; result: PullResult }>("/api/gacha/pull");
      return r.result;
    },
    async pullMulti() {
      const r = await post<{ ok: boolean; result: MultiPullResult }>("/api/gacha/pull10");
      return r.result;
    },
    async daily() {
      const r = await post<{ ok: boolean; result: DailyResult }>("/api/gacha/daily");
      return r.result;
    },
    async balance() {
    ...
    async inventory(opts) {
      const r = await get<{ ok: boolean; page: InventoryPage }>("/api/gacha/inventory/page", {
        rarity: opts?.rarity,
        cursor: opts?.cursor,
        limit: opts?.limit,
      });
    ...
    async sell(cardId) {
      const r = await post<{ ok: boolean; result: SellResult }>("/api/gacha/sell", { cardId });
      return r.result;
    },
    async sellAll() {
      const r = await post<{ ok: boolean; result: SellAllResult }>("/api/gacha/sell-all");
      return r.result;
    },
    async gift(recipientId, cardId) {
      const r = await post<{ ok: boolean; result: GiftResult }>("/api/gacha/gift", {
        recipientId,
        cardId,
      });
      return r.result;
    ...
    async wishlist() {
      const r = await get<{ ok: boolean; items: WishlistItem[] }>("/api/gacha/wishlist");
      return r.items;
    },
    async history(opts) {
      const r = await get<{ ok: boolean; page: HistoryPage }>("/api/gacha/history", {
        cursor: opts?.cursor,
        limit: opts?.limit,
        type: opts?.type,
      });
    ...
    async searchCards(q, limit) {
      const r = await get<{ ok: boolean; items: GachaCard[] }>("/api/gacha/cards/search", {
        q,
        limit,
      });
      return r.items;
    ...
    async badges() {
      const r = await get<{ ok: boolean; progress: BadgeProgress }>("/api/gacha/badges");
      return r.progress;
    },
    async claimBadge() {
      const r = await post<{
    ...
    async fusionPreview() {
      const r = await get<{ ok: boolean; preview: FusionPreview }>("/api/gacha/fusion/preview");
      return r.preview;
    },
    async fuse(cardId) {
      const r = await post<{ ok: boolean; result: FusionResult }>("/api/gacha/fusion", { cardId });
      return r.result;
    },
