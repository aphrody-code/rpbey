...
// --- Font setup ---
try {
  const fontPath = resolveRootPath("public/Google_Sans_Flex/static/GoogleSansFlex_72pt-Bold.ttf");
...
async function safeLoadImage(urlOrPath: string | null): Promise<CanvasImage | null> {
...
async function loadImages(names: string[]): Promise<Map<string, CanvasImage | null>> {
...
function drawBg(ctx: SKRSContext2D, w: number, h: number, bg: CanvasImage | null) {
  if (bg) {
...
function drawTrend(ctx: SKRSContext2D, change: number | "NEW", x: number, y: number) {
...
function footer(ctx: SKRSContext2D, w: number, h: number, scrapedAt: string) {
  ...
  ctx.fillText(`rpbey.fr/meta · bbxweekly.com · Mis à jour le ${updated}`, w / 2, h - 14);
  ctx.textAlign = "left";
}
...
export async function generateMetaTopCanvas(
  ...
  for (let ci = 0; ci < catCount; ci++) {
    ...
    // Parts
    for (let pi = 0; pi < Math.min(cat.components.length, PARTS_PER_CAT); pi++) {
...
export async function generateMetaComboCanvas(
  ...
  for (let i = 0; i < tops.length; i++) {
    ...
    ctx.fillStyle = "#ffffff";
    ctx.fillText(truncText(ctx, comp.name, spacing - 20), cx, partsY + imgR * 2 + 38);
    ...
    // Bar
    drawBar(ctx, cx - 45, partsY + imgR * 2 + 74, 90, 5, comp.score / 100, color);
    ...
    if (comp.synergy?.length) {
      ...
      const synText = comp.synergy
        ...
        .map((s) => s.name)
        .join(" · ");
      ctx.fillText(truncText(ctx, synText, spacing - 20), cx, partsY + imgR * 2 + 95);
    }
  }
  ctx.textAlign = "left";
...
export async function generateMetaPieceCanvas(
  ...
  // Load images
  const allNames = [comp.name, ...comp.synergy.slice(0, synCount).map((s) => s.name)];
  const images = await loadImages(allNames);
