...
const ALLOWED_REDIRECT_HOST_REGEX = /^https:\/\/[a-z0-9-]+\.discordsays\.com(?:\/.*)?$/i;

function isAllowedRedirectUri(uri: string): boolean {
  if (DEFAULT_ALLOWED_REDIRECTS.includes(uri as (typeof DEFAULT_ALLOWED_REDIRECTS)[number]))
    return true;
...
export async function exchangeDiscordCode(
  ...
  if (redirectUri && !isAllowedRedirectUri(redirectUri)) {
    throw new TokenExchangeError("INVALID_REDIRECT_URI", "redirect_uri not in whitelist");
  }
  ...
  if (markCodeSeen(code)) {
    throw new TokenExchangeError("CODE_REPLAY", "OAuth code already consumed (replay detected)");
  }
  ...
  if (!tokenRes.ok) {
    ...
    logger.warn({ status: tokenRes.status, body: safe }, "Discord OAuth token exchange failed");
    throw new TokenExchangeError(
...
export async function handleEntitlementWebhook(args: {
  ...
  if (!publicKey) {
    throw new WebhookError("MISSING_PUBLIC_KEY", "DISCORD_PUBLIC_KEY not configured");
  }
  ...
  if (!Number.isFinite(tsNum)) {
    throw new WebhookError("INVALID_SIGNATURE", "X-Signature-Timestamp is not a number");
  }
  ...
  if (expectedAppId && payload.application_id && payload.application_id !== expectedAppId) {
    logger.warn(
      {
  ...
  try {
    ...
    const alreadyCredited = await prisma.$transaction(async (tx) => {
      ...
      try {
        ...
        // Unique-constraint violation = race avec une autre instance webhook :
        // l'autre a gagné, on traite comme replay. Sous Drizzle/postgres-js le
        // code remonté est le SQLSTATE Postgres `23505` (et non plus le code
        // Prisma `P2002`) — on accepte les deux par robustesse.
    ...
    if (alreadyCredited) {
      logger.info({ entitlementId: evt.data.id }, "IAP entitlement replay — already credited");
      return { status: "ignored" };
    }
  } catch (err) {
  ...
  logger.info({ discordUserId, sku: evt.data.sku_id, amount }, "IAP entitlement credited");
  return {
    status: "credited",
