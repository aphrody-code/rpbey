...
const PARTICIPANTS_MAP_PATH = join(DASHBOARD_EXPORTS_DIR, "participants_map.json");
...
async function loadAliasResolver(): Promise<AliasResolver> {
  try {
    const map = JSON.parse(await readFile(PARTICIPANTS_MAP_PATH, "utf-8")) as Record<
      string,
      MapEntry
    >;
    ...
    for (const [key, e] of Object.entries(map)) {
      ...
      const cands = new Set<string>([e.primaryName ?? "", ...(e.aliases ?? [])]);
    ...
    return {
      resolveKey: (name) => aliasToKey.get(normalizeKey(name)) ?? normalizeKey(name),
...
> {
  ...
  return (playerName: string, challongeUsername: string | null): string | null => {
