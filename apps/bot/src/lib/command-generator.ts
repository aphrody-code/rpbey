...
const COMMAND_TEMPLATE = (name: string, description: string, response: string) => `
...
export async function generateCustomCommands() {
  ...
  try {
    ...
    const srcExists = await Bun.file(path.join(srcDir, ".."))
      ...
      .catch(() => false);
    if (!srcExists && isProd) {
      console.log("Production mode: skipping custom command generation (src not found).");
      return;
    }
    ...
    const hardcodedDirs = ["Beyblade", "General", "Moderation", "Admin", "Music", "Voice"];
    ...
    for (const cmd of commands) {
      ...
      await Bun.write(filePath, COMMAND_TEMPLATE(sanitizedName, cmd.description, cmd.response));
    }
