...
// BTS5-style italic display (Skia synthesizes oblique from GoogleSans-Bold).
// Matches the affiche typography — clean italic bold sans-serif.
const _LB_FONT = (weight: number, size: number) =>
  `italic ${weight} ${size}px GoogleSans, NotoEmoji, sans-serif`;
...
export async function generateLeaderboardCard(
  ...
  const showAvatars = avatars.some((a) => a != null);

  // Layout BTS5 — single column, top 3 plus haut, le reste plus compact.
  const width = 1100;
  const headerH = 230;
  const padX = 36;
  const rowGap = 8;
  ...
  const footerH = 96;
  const bodyPadTop = 24;
  const bodyPadBottom = 24;
  const top3Count = entries.filter((e) => e.rank <= 3).length;
  const restCount = entries.length - top3Count;
  const rowsH =
    top3Count * rowHTop3 +
    Math.max(0, top3Count - 1) * rowGap +
    (restCount > 0 ? rowGap * 2 : 0) +
    restCount * rowHRest +
    Math.max(0, restCount - 1) * rowGap;
  const height = headerH + bodyPadTop + rowsH + bodyPadBottom + footerH;

  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");

  // Fond noir + texture wavy organique (DA BTS5)
  ctx.fillStyle = "#0a0a0a";
  ctx.fillRect(0, 0, width, height);
  drawLbWavyTexture(ctx, 0, headerH, width, height - headerH - footerH);

  // ── Header banner (rouge themé + logo RPB + speckles + sous-titre + diagonale jaune) ──
  await drawLbHeaderBanner(ctx, theme, options, width, headerH);

  // ── Lignes du classement ──
  let y = headerH + bodyPadTop;
  for (const entry of entries) {
    const isTop3 = entry.rank <= 3;
    const h = isTop3 ? rowHTop3 : rowHRest;
    drawLbRow(
      ctx,
      theme,
      padX,
      ...
      h,
      entry,
      avatarByRank.get(entry.rank) ?? null,
      showAvatars,
      { showWL, showParticipations },
    );
    y += h + rowGap;
    // Sépareur visuel entre top 3 et reste
    if (entry.rank === 3 && entries.some((e) => e.rank > 3)) {
      y += rowGap;
    }
  }

  // ── Footer (bandeau noir + diagonale jaune full-width) ──
  drawLbFooterBanner(ctx, theme, width, height - footerH, footerH);
...
type LbVariant = "rpb" | "wb" | "satr" | "stardust";
...
interface LbVariantTheme {
  ...
  badge: string;
  footerUrl: string;
  /** Fond de la carte (carrément le panneau intérieur). */
  bgFill: string;
  /** Couleur des rangées paires (alternance). */
  rowAlt: string;
  /** Couleur de la pill du rank #N (et autres placeholders sombres). */
  rankPill: string;
}
...
const LB_VARIANT_THEMES: Record<LbVariant, LbVariantTheme> = {
  ...
  stardust: {
    primary: "#60a5fa",
    secondary: "#a5b4fc",
    barStops: ["#0c1730", "#1e3a5f", "#60a5fa"],
    title: "STARDUST SERIES",
    badge: "RPB · STARDUST",
    footerUrl: "rpbey.fr/tournaments/stardust",
    // Bleu marine aligné sur le radial gradient de /tournaments/stardust
    bgFill: "#0a1a2e",
    rowAlt: "#0f243f",
    rankPill: "#1e3a5f",
  },
...
function drawLbWavyTexture(ctx: CanvasCtx, x: number, y: number, w: number, h: number) {
  ...
  for (let row = 0; row < Math.ceil(h / stepY) + 2; row++) {
    const yy = y - 60 + row * stepY;
    const phase = (row * seedShift) % period;
    ctx.beginPath();
    for (let xx = x - 40; xx <= x + w + 40; xx += 8) {
      const k = (xx + phase) / period;
      const yOff =
        Math.sin(k * Math.PI * 2) * amp * 0.5 + Math.sin(k * Math.PI * 2 * 1.7 + row) * amp * 0.25;
      if (xx === x - 40) ctx.moveTo(xx, yy + yOff);
      else ctx.lineTo(xx, yy + yOff);
    }
    ctx.stroke();
  }
  ctx.restore();
...
async function drawLbHeaderBanner(
  ctx: CanvasCtx,
  ...
  // Particules jaunes éparses (signature affiche BTS5)
  drawLbSpeckles(ctx, 0, 0, w, h - 28, LB_YELLOW, 28, 13);

  const padX = 40;
  ctx.textAlign = "left";
  ctx.textBaseline = "alphabetic";

  // Logo RPB haut-droite (etoile rouge + tete dragon dore).
  // Load directly avec loadImage pour preserver l'alpha channel
  // (safeLoadImage applique removeWhiteBackground sur les webp et
  // peut corrompre les sprites a fond transparent).
  const logoSize = h - 50;
  let logo: CanvasImage | null = null;
  try {
    logo = await loadImage(getAssetPath("assets/rpb-logo.webp"));
  } catch {
    // Logo absent — skip silently
  }
  if (logo) {
  ...
  // URL en petit minuscule (police GoogleSans dashboard)
  ctx.font = _FONT("bold", 20);
  ctx.fillStyle = "rgba(255,255,255,0.85)";
  ctx.fillText(theme.footerUrl, padX, 50);

  // Sous-titre saison + tournois compté en gros mixed-case, ajuste auto a la largeur
  const subtitle = options.subtitle ?? theme.badge;
  const maxSubtitleW = w - padX * 2 - (logo ? logoSize + 20 : 0);
  let subtitleSize = 56;
  while (subtitleSize > 28) {
    ctx.font = _FONT("bold", subtitleSize);
    if (ctx.measureText(subtitle).width <= maxSubtitleW) break;
    subtitleSize -= 2;
  }
  ctx.fillStyle = "#fff";
  ctx.fillText(subtitle, padX, 130);

  // Diagonale jaune full-width (signature affiche BTS5)
  drawLbDiagonal(ctx, w, h - 12, 20, LB_YELLOW);
  drawLbDiagonal(ctx, w, h - 6, 4, "#000");
}

function drawLbFooterBanner(
  ctx: CanvasCtx,
  theme: LbVariantTheme,
  w: number,
  y: number,
  ...
  // Bandeau noir + liseré themé top
  ctx.fillStyle = "#000";
  ctx.fillRect(0, y, w, h);
  ctx.fillStyle = theme.primary;
  ctx.fillRect(0, y, w, 2);

  // Diagonale jaune full-width
  drawLbDiagonal(ctx, w, y + h - 18, 14, LB_YELLOW);

  // Date seule, GoogleSans dashboard
  const padX = 40;
  ...
  ctx.fillText(date, w - padX, y + h / 2 - 6);
}

function drawLbSquareAvatar(
  ctx: CanvasCtx,
  ...
  if (img) {
    ...
    ctx.font = _LB_FONT(900, Math.floor(size * 0.55));
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(fallbackInitial.toUpperCase(), x + size / 2, y + size / 2 + 2);
  }
  ctx.restore();
...
function lbRankAccent(rank: number, themePrimary: string): string {
  if (rank === 1) return MEDAL[0];
  if (rank === 2) return MEDAL[1];
  if (rank === 3) return MEDAL[2];
  if (rank <= 10) return themePrimary;
  return "#5a5570";
}
...
function drawLbRow(
  ...
  avatar: CanvasImage | null,
  showAvatars: boolean,
  cols: { showWL: boolean; showParticipations: boolean },
) {
  const accent = lbRankAccent(entry.rank, theme.primary);
  const isTop3 = entry.rank <= 3;

  // Fond ligne — rectangle plein noir (DA BTS5, pas de border-radius)
  ctx.fillStyle = "rgba(0,0,0,0.92)";
  ctx.fillRect(x, y, w, h);

  // Liseré themé fin (signature BTS5 cartes prix)
  ctx.strokeStyle = isTop3 ? accent : theme.primary;
  ctx.lineWidth = isTop3 ? 2.5 : 1.5;
  ctx.strokeRect(x + 1, y + 1, w - 2, h - 2);

  // Surbrillance gauche pour top 3 (rectangle plein)
  if (isTop3) {
    ctx.fillStyle = accent;
  ...
  // Numéro + # postfixé (signature affiche "1#" "2#" "3#")
  ctx.font = _LB_FONT(900, isTop3 ? 70 : 50);
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  ctx.fillStyle = isTop3 ? accent : "rgba(255,255,255,0.88)";
  const rankX = x + padL;
  const rankStr = `${entry.rank}#`;
  ctx.fillText(rankStr, rankX, cy + 2);
  const rankW = ctx.measureText(rankStr).width;

  // Avatar carré (DA BTS5)
  let textX = rankX + rankW + 22;
  if (showAvatars) {
    const avSize = h - 22;
    const avX = rankX + rankW + 22;
    const avY = y + (h - avSize) / 2;
    drawLbSquareAvatar(ctx, avatar, avX, avY, avSize, entry.name.slice(0, 1), accent);
    textX = avX + avSize + 20;
  }

  // Reserve droite pour points + stats
  const statsX = x + w - padL;
  const statsBlockW = isTop3 ? 320 : 280;
  const nameMaxW = statsX - statsBlockW - textX;

  ctx.font = _LB_FONT(800, isTop3 ? 32 : 26);
  ctx.fillStyle = "#fff";
  ctx.textAlign = "left";
  ctx.textBaseline = "alphabetic";
  const name = entry.name.length > 22 ? `${entry.name.substring(0, 22)}…` : entry.name;
  const measuredName = ctx.measureText(name).width;
  const finalName =
    measuredName > nameMaxW
      ? `${name.substring(0, Math.max(1, Math.floor(name.length * (nameMaxW / Math.max(1, measuredName))) - 1))}…`
      : name;
  ctx.fillText(finalName, textX, cy - 4);

  // Sous-ligne stats V/D/WR + participations
  ctx.font = _LB_FONT(600, isTop3 ? 16 : 14);
  ctx.fillStyle = "rgba(255,255,255,0.65)";
  const wrVal = parseFloat(`${entry.winRate}`) || 0;
  const wins = entry.wins ?? 0;
  const losses = entry.losses ?? 0;
  const stats: string[] = [];
  if (cols.showWL || wins + losses > 0) {
    stats.push(`${wins}V — ${losses}D`);
  }
  stats.push(`${wrVal}% WR`);
  if (cols.showParticipations && typeof entry.participations === "number") {
    stats.push(`${entry.participations} part.`);
  }
  ctx.fillText(stats.join(" · "), textX, cy + (isTop3 ? 22 : 18));

  // Points (droite, gros chiffre themé/accent)
  ctx.font = _LB_FONT(900, isTop3 ? 50 : 40);
  ctx.textAlign = "right";
  ctx.fillStyle = accent;
  ctx.fillText(entry.points.toLocaleString("fr-FR"), statsX, cy + 4);

  // Label POINTS
  ctx.font = _LB_FONT(700, 12);
  ctx.fillStyle = "rgba(255,255,255,0.5)";
  ctx.fillText("POINTS", statsX, cy + (isTop3 ? 26 : 22));
}
