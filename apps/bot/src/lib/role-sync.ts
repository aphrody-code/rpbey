		...
		if (rolesToAdd.length > 0) {
			addJobs.push(async () => {
				try {
					...
					logger.info(
						`[${taskName}] + roles [${rolesToAdd.join(", ")}] → ${member.user.tag}`,
					);
				} catch (err) {
					logger.error(
						`[${taskName}] Error adding roles to ${member.user.tag}:`,
						err,
					);
				}
			});
		...
		if (rolesToRemove.length > 0) {
			removeJobs.push(async () => {
				try {
					...
					logger.info(
						`[${taskName}] - roles [${rolesToRemove.join(", ")}] ← ${member.user.tag}`,
					);
				} catch (err) {
					logger.error(
						`[${taskName}] Error removing roles from ${member.user.tag}:`,
						err,
					);
				}
			});
