...
// Use shared type (eliminates duplicate definition)
type RankingConfig = RankingPointsConfig;
// Re-use shared bucket map (same values, no duplication)
const POINTS_BY_FINISH = FINISH_BUCKET_MAP;
...
function isTrustworthyForPlacements(t: StardustTournament): boolean {
  const okStatus =
    t.status === "COMPLETE" || t.status === "ARCHIVED" || t.challongeState === "complete";
...
export function buildStardustRankings(
  ...
  // Sort: points desc, tournamentWins desc, wins desc (shared canonical order)
  const sorted = [...players.entries()].sort(([, a], [, b]) => compareRankEntries(a, b));
  ...
  const ranked = sorted.map(([, p], i) => {
    ...
    const pointsAverage =
      p.tournaments.size > 0 ? (p.points / p.tournaments.size).toFixed(2) : "0.00";
    void nbT;
    return {
      rank: i + 1,
...
> {
  try {
    const tournaments = (await prisma.tournament.findMany({
      where: {
        category: { name: { contains: "STARDUST", mode: "insensitive" } },
        status: { in: ["COMPLETE", "ARCHIVED", "UNDERWAY"] },
      },
      orderBy: { date: "asc" },
      include: { participants: true, matches: true, category: true },
    })) as unknown as StardustTournament[];
    ...
    // Stardust firstPlace default is 15 000 (differs from BTS 10 000)
    const config = await loadPointsConfig(prisma, { firstPlace: 15000 });

    const { ranked, playerStats, bladerHistory } = buildStardustRankings(tournaments, config);

    // NOTE: the Drizzle façade's array-form `$transaction([a, b])` awaits
    // already-dispatched promises, so it cannot guarantee delete-before-insert
    // ordering. Use the callback form, which opens a real serial transaction.
    await prisma.$transaction(async (tx) => {
      await tx.stardustRanking.deleteMany();
      await tx.stardustRanking.createMany({ data: ranked });
    });

    for (const [k, stats] of playerStats.entries()) {
      const hist = (bladerHistory.get(k) ?? []) as unknown as object[];
      await prisma.stardustBlader.upsert({
        where: { name: stats.displayName },
        update: {
