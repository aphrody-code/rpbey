...
redis
  ...
  .catch((err: Error) => logger.error("[Redis] Connection error:", err.message));
...
export async function setScanMeta(channelsScanned: number, messagesScanned: number): Promise<void> {
  await redis.hset("rpb:mentions:meta", "channelsScanned", String(channelsScanned));
  await redis.hset("rpb:mentions:meta", "messagesScanned", String(messagesScanned));
...
export async function getScanMeta(): Promise<{
  ...
  const data = (await redis.hgetall("rpb:mentions:meta")) as Record<string, string>;
  return {
