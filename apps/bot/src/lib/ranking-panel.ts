...
import { cached, cacheDel, TTL } from "./cache.js";
...
export type RankingVariant = "rpb" | "wb" | "satr" | "stardust";
...
export function decodeAction(
  ...
  // Format : `rnk:<action>:<variant>:<season>:<page>` (5 parts) ou
  // `rnk:page:<variant>:<season>:<page>:<p|n>` (6 parts) pour les boutons
  // pagination (suffixe directionnel pour éviter COMPONENT_CUSTOM_ID_DUPLICATED).
  ...
  if (variant !== "rpb" && variant !== "wb" && variant !== "satr" && variant !== "stardust")
    return null;
 ...
 *
 * Results are cached in Redis (TTL.STATIC = 24h) since seasons change rarely.
 * Call `invalidateSeasonsCache(variant)` when a ranking cron rewrites rows.
 */
export async function listSeasons(variant: RankingVariant): Promise<SeasonRef[]> {
  return cached(`seasons:${variant}`, TTL.STATIC, () => _listSeasonsRaw(variant));
}

async function _listSeasonsRaw(variant: RankingVariant): Promise<SeasonRef[]> {
  if (variant === "rpb") {
  ...
  if (variant === "stardust") {
    // Stardust ranking is a single global series (T_SS1 → ...).
    return [{ key: "current", label: "Stardust Series", sublabel: "T_SS1+" }];
  }
  if (variant === "wb") {
...
/**
 * Invalidate the seasons cache for a specific variant (or all variants).
 * Should be called by any cron that writes new ranking rows.
 */
export async function invalidateSeasonsCache(...variants: RankingVariant[]): Promise<void> {
  const targets =
    variants.length > 0 ? variants : (["rpb", "wb", "satr", "stardust"] as RankingVariant[]);
  await cacheDel(...targets.map((v) => `seasons:${v}`));
}
...
async function loadEntries(
  ...
  if (variant === "stardust") {
    const [rows, total] = await Promise.all([
      prisma.stardustRanking.findMany({
        take,
        skip,
        orderBy: { rank: "asc" },
      }),
      prisma.stardustRanking.count(),
    ]);
    return {
      total,
...
> = {
  ...
  stardust: {
    ...
    emoji: "✨",
    color: 0x60a5fa,
    url: "https://rpbey.fr/tournaments/stardust",
  },
};
...
export async function renderRankingPanel(state: PanelState): Promise<PanelRenderResult> {
    ...
    new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
      new StringSelectMenuBuilder()
        ...
        .addOptions(
          ...
          {
            label: "Stardust Series",
            value: "stardust",
            default: variant === "stardust",
            emoji: "✨",
          },
        ),
    ),
  ...
  // Pagination buttons.
  // Important : Discord rejette les composants ayant le même customId dans
  // le même message (`COMPONENT_CUSTOM_ID_DUPLICATED`). Quand totalPages === 1
  // le prev (page 0) et le next (page totalPages-1 = 0) auraient le même
  // customId encodé. On ajoute donc un suffixe directionnel `:p` / `:n`.
  const prevPage = Math.max(0, page - 1);
  const nextPage = Math.min(totalPages - 1, page + 1);
  const prevId = `${encodeAction("page", { ...state, page: prevPage })}:p`;
  const nextId = `${encodeAction("page", { ...state, page: nextPage })}:n`;
