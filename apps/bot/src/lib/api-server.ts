...
const scopedBuckets = new Map<string, Map<string, { count: number; startTime: number }>>();
...
function checkRateLimitFor(scope: string, ip: string, max: number, windowMs: number): boolean {
...
const PLAY_BUNDLE_DIR =
  process.env.GACHA_CLIENT_DIST ?? new URL("../../../gacha-client/dist", import.meta.url).pathname;
...
function buildServer(port: number): Server<WsData> {
  ...
  return Bun.serve<WsData>({
    ...
    routes: {
      ...
      "/health": () =>
        Response.json({ status: "ok", uptime: process.uptime() }, { headers: CORS_HEADERS }),
      // `/api/health` est défini par `getRefactorRoutes()` (W1A) avec un
      // schema enrichi `{ ok, uptime, commit }` — fusionné via spread plus bas.
      "/ready": () => {
      ...
      "/api/discord/token-exchange": {
        POST: async (req: Request, srv: Server<WsData>) => {
          ...
          try {
            ...
            if (typeof body.code !== "string" || body.code.length < 10 || body.code.length > 1024) {
              return Response.json(
            ...
            if (
              ...
              (typeof body.redirect_uri !== "string" || body.redirect_uri.length > 512)
            ) {
              return Response.json(
                {
                  error: "BAD_REQUEST",
      ...
      // Refacto Vercel (W1A) — scrapers/tournaments/rankings stubs Bearer-auth
      // + /api/health enrichi. Voir `apps/rpb-bot/src/api/server.ts`.
      ...getRefactorRoutes(),
        ...
        POST: async (req: Request, srv: Server<WsData>) => {
          ...
          try {
            ...
            if (err instanceof WebhookError) {
              const status =
                err.code === "INVALID_SIGNATURE" ? 401 : err.code === "USER_NOT_FOUND" ? 404 : 500;
              return Response.json(
                { error: err.code, message: err.message },
