...
const COUNT_REL: Record<
	string,
	Record<string, { table: keyof typeof schema; fk: string }>
> = {
	tournament: {
...
function buildCondition(table: Any, where: Row | undefined): Any {
	...
	for (const [field, raw] of Object.entries(where)) {
		...
		if (field === "OR") {
			...
			if (ors.length)
				conds.push(
					ors.length === 1
						? ors[0]
						: (sql`(${sql.join(ors, sql` OR `)})` as Any),
				);
			continue;
		}
		if (field === "NOT") {
				...
				if (localCol && relCond) {
					conds.push(
						inArray(
							localCol,
							(db as Any)
								.select({ id: relTable[relFilter.relPk] })
								.from(relTable)
								.where(relCond),
						),
					);
		...
		if (raw === null) {
			...
			if ("startsWith" in op)
				conds.push((insensitive ? ilike : like)(col, `${op.startsWith}%`));
			if ("endsWith" in op)
...
function buildDrizzleWith(
	...
	for (const [prismaKey, val] of Object.entries(relSource)) {
		...
		if (val === true) {
			...
			const inner: Row = {};
			if (sub.where && childTable)
				inner.where = buildCondition(childTable, sub.where);
			if (sub.orderBy && childTable)
				inner.orderBy = buildOrderBy(childTable, sub.orderBy);
...
function realiasRow(
	...
	if (relSource) {
		for (const [prismaKey, val] of Object.entries(relSource)) {
			...
			const childModel = relModelOf(model, prismaKey);
			const sub =
				(typeof val === "object" ? (val as FindArgs) : undefined) ?? undefined;
...
function makeDelegate(model: string, executor: DB | Tx) {
	...
	async function read(args: FindArgs, single: boolean): Promise<Any> {
		...
		const needsRelations =
			relSource &&
			Object.keys(relSource).some((k) => (INCLUDE_ALIAS[model] ?? {})[k]);
		...
		if (needsRelations || wantsCount) {
			...
			let mapped = rows.map(
				(r) => realiasRow(model, r, args.include, args.select)!,
			);
			...
			if (args.select) {
				const scalarKeys = Object.keys(args.select).filter(
					(k) =>
						args.select![k] &&
						!(INCLUDE_ALIAS[model] ?? {})[k] &&
						k !== "_count",
				);
				const relKeys = Object.keys(args.select).filter(
					(k) => (INCLUDE_ALIAS[model] ?? {})[k],
				);
				mapped = mapped.map((r) => {
					const o: Row = {};
		...
		if (distinctCols.length) {
			...
			const distinctOrder = distinctCols.map((c) =>
				dirOf(c) === "desc" ? desc(c) : asc(c),
			);
		...
		const projected = distinctCols.length
			? rows
			: applySelect(rows, args.select);
	...
	async function insertWithNested(data: Row): Promise<Row> {
		...
		for (const n of nested) {
			...
			if (childRows.length)
				await (executor as Any).insert(childTable).values(childRows);
		}
		return created;
	}
	...
	return {
		...
		async createMany(args: { data: Row[] }): Promise<{ count: number }> {
			...
			const rows = args.data.map((d) =>
				ensureId(table, buildValues(table, d).values),
			);
		...
		async upsert(args: {
			...
			let q = (executor as Any)
				.insert(table)
				.values(ensureId(table, createValues));
			...
			if ((args.include || args.select) && row) {
				return read(
					{ where: { id: row.id }, include: args.include, select: args.select },
					true,
				);
			}
			return row;
		...
		async delete(args: { where: Row }): Promise<Any> {
			const cond = buildCondition(table, args.where);
			const [deleted] = await (executor as Any)
				.delete(table)
				.where(cond)
				.returning();
			return deleted;
