import { publishEvent } from "../../lib/events-pubsub.js";
...
export async function rankingSyncTask() {
	...
	const apiUrl =
		process.env.RPB_API_URL?.replace(/\/$/, "") ?? "https://rpbey.fr";
	...
	try {
		...
		if (!res.ok || !body.ok) {
			logger.error(
				`[Cron] Ranking sync failed (HTTP ${res.status}) : ${JSON.stringify(body)}`,
			);
			return;
		}
		...
		void publishEvent("ranking", {
			ts: Date.now(),
			updated: (body.results ?? []).filter((r) => r.success).length,
			summary,
		});
	} catch (err) {
