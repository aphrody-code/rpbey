import { ChannelType, type Collection, type Message, type Snowflake } from "discord.js";
...
export async function mentionsScanTask() {
  ...
  // Write to Redis — single multi-field HSET per chunk (one round-trip per 500 pairs)
  try {
    await clearMentions();
    ...
    for (let i = 0; i < entries.length; i += BATCH_SIZE) {
      ...
      // Build flat [field, value, field, value, …] array for HSET multi-field
      const args: string[] = ["rpb:mentions"];
      for (const [key, count] of batch) {
        args.push(key, String(count));
      }
      // redis.send('HSET', [...]) — single round-trip, replaces N individual hset calls
      await redis.send("HSET", args);
    }
