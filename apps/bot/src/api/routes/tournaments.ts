...
const liveRefresh = withAuth<{ id: string }>(async (req) => {
	...
	if (!tournament)
		return errorResponse("NOT_FOUND", "Tournament not found", 404);
	...
	if (!challongeRef)
		return errorResponse(
			"BAD_REQUEST",
			"Tournament not linked to Challonge",
			400,
		);
	...
	if (!apiKey)
		return errorResponse(
			"MISSING_ENV",
			"CHALLONGE_API_KEY not configured",
			500,
		);
...
const sync = withAuth(async (req) => {
	...
	if (transport === "api" && !process.env.CHALLONGE_API_KEY) {
		return errorResponse(
			"MISSING_ENV",
			"CHALLONGE_API_KEY missing (transport='api' forced)",
			500,
		);
	}
	...
	if (transport === "api") {
		...
		try {
			...
			logger.warn(
				{ err },
				"[sync] api transport failed, falling back to htmlrewriter",
			);
			result = await fetchViaHtmlRewriter();
		}
	} else {
...
const refreshBrackets = withAuth(async (req) => {
	const { body, error } = await readJsonBody<
		BracketsTransportBody & { tournamentId?: string }
	>(req);
	if (error) return error;
	...
	if (!slug)
		return errorResponse(
			"BAD_REQUEST",
			"idOrSlug or tournamentId required",
			400,
		);
...
const finalize = withAuth(async (req) => {
	...
	if (scraped) {
		...
		for (const p of scraped.participants) {
			...
			challongeIdToUser.set(
				p.id,
				userByKey.get(cleanName.toLowerCase()) ?? null,
			);
		}
		...
		await prisma.tournament.update({
			...
			data: {
				...
				name: keepName
					? tournament.name
					: (scraped.metadata.name ?? tournament.name),
			},
		});
		...
		for (const m of scraped.matches) {
			const player1Name = m.player1Id
				? (challongeIdToName.get(m.player1Id) ?? null)
				: null;
			const player2Name = m.player2Id
				? (challongeIdToName.get(m.player2Id) ?? null)
				: null;
			const winnerName = m.winnerId
				? (challongeIdToName.get(m.winnerId) ?? null)
				: null;
			const player1Uid = m.player1Id
				? (challongeIdToUser.get(m.player1Id) ?? null)
				: null;
			const player2Uid = m.player2Id
				? (challongeIdToUser.get(m.player2Id) ?? null)
				: null;
			const winnerUid = m.winnerId
				? (challongeIdToUser.get(m.winnerId) ?? null)
				: null;
	...
	try {
		...
		logger.warn(
			{ err },
			"[finalize] ranking helpers not present yet (W2D pending)",
		);
		syncResult = {
			triggered: "skipped",
			success: false,
	...
	let publishResult:
		| Awaited<
				ReturnType<
					typeof import("../../lib/classement-publisher.js").publishBtsRanking
				>
		  >
		| { ok: false; skipped: true } = { ok: false, skipped: true };
	if (body.publishRanking) {
		try {
			const { publishBtsRanking } = await import(
				"../../lib/classement-publisher.js"
			);
