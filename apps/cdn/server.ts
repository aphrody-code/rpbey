...
const FALLBACK_PUBLIC_BASE =
	process.env.CDN_PUBLIC_BASE ?? "https://cdn.rosegriffon.fr";
const ASSETS_MANIFEST =
	process.env.CDN_ASSETS_MANIFEST ?? "/var/www/cdn/assets-manifest.json";
...
const ASSET_SCOPES: Record<string, string> = {
	...
	"rpb-bey-library": pathResolve(
		REPO_ROOT,
		"apps/cdn/assets/rpb-bey-library/images",
	),
};
...
function classify(path: string): AssetKind {
	...
	if (ext === ".mp3" || ext === ".ogg" || ext === ".oga" || ext === ".wav")
		return "audio";
