...
let ADMIN_TOKEN = "";
try {
  ADMIN_TOKEN = readFileSync(
    process.env.ADMIN_TOKEN_FILE ?? "/home/ubuntu/.rpb-adm-session",
    "utf8",
  ).trim();
} catch {}
...
function walk(dir: string, seg = ""): string[] {
  ...
  for (const e of readdirSync(dir)) {
    ...
    if (files.includes("route.ts") || files.includes("route.tsx"))
      out.push("API " + (child || "/"));
...
const benign = (u: string) =>
  /_rsc=|cdn\.discordapp|user-assets\.challonge|googleusercontent|ytimg|takaratomy|\/api\/auth\/get-session|429|Too Many Requests|net::ERR_FAILED|ERR_ABORTED|status of 404 \(\)/.test(
    u,
  );
...
for (const route of pages) {
  ...
  page.on("console", (m) => {
    const t = m.text();
    if (m.type() === "error" && !/Failed to load resource/.test(t) && !benign(t))
      errors.push("console " + t.slice(0, 160));
  });
  page.on("response", (r) => {
    if (r.status() >= 400 && r.url().startsWith(BASE) && !benign(r.url()))
      errors.push(`${r.status()} ${r.url().slice(BASE.length).slice(0, 90)}`);
  });
  let status = 0;
  try {
    ...
    errors.push("GOTO " + String((e as Error).message).slice(0, 120));
  }
  await new Promise((r) => setTimeout(r, 250)); // évite le rate-limit get-session sous charge
  const name = (route === "/" ? "home" : route.replace(/[/?=&]/g, "_").replace(/^_/, "")).slice(
    0,
    80,
  );
  ...
  if (bad)
    fails.push(
      `PAGE ${status} ${route}${authedRedirect ? " (auth NON appliquée → redirect sign-in)" : ""}\n    ${errors.slice(0, 4).join("\n    ")}`,
    );
  console.log(
    `${bad ? "✗" : "✓"} ${status} ${route}${errors.length ? `  (${errors.length} err)` : ""}`,
  );
  await page.close();
}
...
for (const route of apis) {
  let status = 0,
    body = "";
  try {
    if (/\/api\/(events|bot\/events)/.test(route)) {
      console.log("- skip SSE " + route);
      continue;
    }
    ...
    body = String((e as Error).message).slice(0, 80);
  }
