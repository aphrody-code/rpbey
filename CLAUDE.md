...
- **⚠️ `bun run build:web` SEGFAULT/SIGILL** (Bun 1.3.14, à la phase « Running TypeScript », reproductible — PAS un OOM, peak ~2 GB) → **builder via node** : `cd apps/web && node ../../node_modules/.bin/next build --turbopack` (node v24 dans `~/.local/bin`). Passe la phase TS sans crash, standalone COMPLET (un `panic` Bun cosmétique peut s'afficher en toute fin, ignorer si `Generating static pages (N/N)` + `.next/BUILD_ID` présents). Puis `deploy-web.sh` + restart + vérifier l'epoch BUILD_ID ≥ service start (sinon stale).
